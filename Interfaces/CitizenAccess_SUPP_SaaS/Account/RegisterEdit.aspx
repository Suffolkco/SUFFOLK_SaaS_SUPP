...
<asp:Content ID="Content1" ContentPlaceHolderID="PlaceHolderMain" runat="Server">
    ...
    <div id="divACADialogLayerMaskForRegister" class="ACA_MaskDiv" style="display: none; height: 1201px; z-index: 998;">
        <iframe class="mask_iframe" title="This is a masked layout for Silverlight control."></iframe>
    </div>
     <!-- The warning message box show up when add a new contact in register page and the new contact don't exsit in reference end-->

        <div class="ACA_Dialog" id="divConfirmAddContact" style='display: none; min-height: 100px; height: 100px; width: 360px; z-index: 999; padding:10px 5px 0px 5px;'>
            <table>
                <tr>
                    <td style='height: 50PX; vertical-align:middle;' class="ACA_SmLabel ACA_SmLabel_FontSize">
                        <ACA:AccelaLabel ID="lblAddNewConfirmInformation" LabelKey="aca_publicuser_contact_add_warning" runat="server" />
                    </td>
                </tr>
                <tr>
                    <td class="ACA_Row ACA_LiLeft">
						<ul>
                            <li>
								<div class="ACA_LgButton ACA_LgButton_FontSize" dir="rtl">
									<ACA:AccelaButton ID="btnConfirmAddContact" href="javascript:;" runat="server" LabelKey="aca_create_new_register_contact_button_continue" OnClientClick="confirmAddContact(); return false;">
									</ACA:AccelaButton>
								</div>
							</li>
							<li>
								 <div class="ACA_LgButton ACA_LgButton_FontSize" dir="rtl">
									<ACA:AccelaButton ID="btnCancelAddContact" OnClientClick="cancelAddContact();return false;" runat="server" LabelKey="aca_create_new_register_contact_button_cancel"  >
									</ACA:AccelaButton>
								</div>
						  </li>
                        </ul>
    ...
    <div id="divRegisterEdit" class="ACA_Content">
        ...
        <div id="divRecaptcha" runat="server">
            <ACA:Recaptcha ID="reCaptcha" SizeMode="normal" runat="server" />
        </div>
    ...
    <script type="text/javascript">
	    // User enters contact info and clicks continue.  
        // If no match is found display warning message.
	   	function popupConfirmDialog() {
            var dvACADialogLayer = $("#dvACADialogLayer");
            var zIndex = dvACADialogLayer.css("z-index");

            //recaculate the width and height
            if (zIndex != undefined && !isNaN(zIndex)) {
                $("#divACADialogLayerMaskForRegister").css("z-index", parseInt(zIndex) + 1);
                $("#divConfirmAddContact").css("z-index", parseInt(zIndex) + 2);
                $("#divACADialogLayerMaskForRegister").width(dvACADialogLayer.width());
                $("#divACADialogLayerMaskForRegister").height(dvACADialogLayer.height());
                $("#divACADialogLayerMaskForRegister").css({ "left": dvACADialogLayer.css("left"), "top": dvACADialogLayer.css("top") });

                var popupIdLeft = dvACADialogLayer.css("left");
                popupIdLeft = popupIdLeft.replace("px", "");
                popupIdLeft = parseInt(dvACADialogLayer.width()) / 2 - $("#divConfirmAddContact").width() / 2;

                var popupIdTop = dvACADialogLayer.css("top");
                popupIdTop = popupIdTop.replace("px", "");
                popupIdTop = parseInt(dvACADialogLayer.height()) / 2;

                $("#divConfirmAddContact").css({ "left": popupIdLeft + "px", "top": popupIdTop + "px" });
            }

            $("#divConfirmAddContact").show();
            $("#divACADialogLayerMaskForRegister").show();
        }

         //Confirm add contact.
        function confirmAddContact() {
            isClickAddContact = true;
            $("#divACADialogLayerMaskForRegister").hide();
            $("#divConfirmAddContact").hide();
            ACADialogFrame.window.PopupClose("N");
        }
        ...
        function cancelAddContact() {
            ...
            $("#divACADialogLayerMaskForRegister").hide();
            $("#divConfirmAddContact").hide();
            //clear the register contact session
            $.ajax({                       
                type: "Post",           
                url: "RegisterEdit.aspx/ClearRegisterContact",
