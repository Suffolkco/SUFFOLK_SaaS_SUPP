...
function getInspectionInfo(fieldNum) {
    ...
    if (isNaN(fieldNum)) {
        ...
        var resultArr = aadb.dbDataSet("SELECT B1_PER_ID1,B1_PER_ID2,B1_PER_ID3, G6_ACT_NUM FROM GGUIDESHEET WHERE SERV_PROV_CODE='SUFFOLKCO' AND GUIDE_TYPE='Lab Methods' and GUIDESHEET_ID='" + fieldNum + "'", 2);
        if (resultArr.length > 1) {
            inspRecordId = aa.cap.getCapID(resultArr[1]["B1_PER_ID1"], resultArr[1]["B1_PER_ID2"], resultArr[1]["B1_PER_ID3"]).getOutput();
            inspSeqNum = resultArr[1]["G6_ACT_NUM"];
        }
        else {
            // try removing the dashes
            var resultArr3 = aadb.dbDataSet("SELECT B1_PER_ID1,B1_PER_ID2,B1_PER_ID3, G6_ACT_NUM FROM GGUIDESHEET WHERE SERV_PROV_CODE='SUFFOLKCO' AND GUIDE_TYPE='Lab Methods' and GUIDESHEET_ID='" + fieldNum.split('-').join('') + "'", 2);
            if (resultArr3.length > 1) {
                inspRecordId = aa.cap.getCapID(resultArr3[1]["B1_PER_ID1"], resultArr3[1]["B1_PER_ID2"], resultArr3[1]["B1_PER_ID3"]).getOutput();
                inspSeqNum = resultArr3[1]["G6_ACT_NUM"];
            }
        }
    }
    else {
        // either not legacy or legacy sample bottle numbers. Legacy bottle numbers are numeric but match the guidesheet ID
        var resultArr2 = aadb.dbDataSet("SELECT B1_PER_ID1,B1_PER_ID2,B1_PER_ID3, G6_ACT_NUM FROM GGUIDESHEET WHERE SERV_PROV_CODE='SUFFOLKCO' AND GUIDE_TYPE='Lab Methods' and GUIDESHEET_ID='" + fieldNum + "'", 2);
        if (resultArr2.length > 1) {
            inspRecordId = aa.cap.getCapID(resultArr2[1]["B1_PER_ID1"], resultArr2[1]["B1_PER_ID2"], resultArr2[1]["B1_PER_ID3"]).getOutput();
            inspSeqNum = resultArr2[1]["G6_ACT_NUM"];
        }
        else {
            // get the inspection according to the fieldnum value
            var resultArr = aadb.dbDataSet("SELECT B1_PER_ID1,B1_PER_ID2,B1_PER_ID3, G6_ACT_NUM FROM G6ACTION WHERE SERV_PROV_CODE='SUFFOLKCO' AND G6_ACT_NUM=" + fieldNum, 2);
            if (resultArr.length > 1) {
                inspRecordId = aa.cap.getCapID(resultArr[1]["B1_PER_ID1"], resultArr[1]["B1_PER_ID2"], resultArr[1]["B1_PER_ID3"]).getOutput();
                inspSeqNum = resultArr[1]["G6_ACT_NUM"];  // same value as fieldNum. Retrieving from table to make it have a consistent type as the other results
            }
        }
    }
