...
//if no structure exists then add one, else update
/*
eval(getScriptText("EH_FacilityStructureCustomFunctions"));
var flagStructureExists = false;
var capStructures = aa.bStructure.getStructure(capId, 1);
if (capStructures.getSuccess()) {
	capStructures = capStructures.getOutput();
	logDebug("getStructures Success, length=" + capStructures.length);
    flagStructureExists = true;
} else {
	logDebug("getStructures ERROR: " + capStructures.getErrorMessage());
}
if(flagStructureExists){
    var programIdentifierString = getAppSpecific("Program Identifier");
    if(!matches(programIdentifierString,null,undefined,"")){
        updateBStructure(capId,1,"FACILITY",programIdentifierString,programIdentifierString)
    }
}else{
    var programIdentifierString = getAppSpecific("Program Identifier");
    if(!matches(programIdentifierString,null,undefined,"")){
