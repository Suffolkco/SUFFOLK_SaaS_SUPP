/*------------------------------------------------------------------------------------------------------/
| Program:DCA_DOCKET_PAYMENT_DUE.js: Batch
| 
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - DCA_DOCKET_PAYMENT_DUE", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
		...
		// Pull Docket Type with payment due date for today
						...
						for (c in capContacts) 
						{
							if (capContacts[c].getCapContactModel().getContactType() == "Vendor")
							{
								...
								{						
									...
									firstName = capContacts[c].getFirstName();									
									...
									sendNotification("", conEmail, "", "DCA_DOCKET_PAYMENT_DUE", emailParams, null);
								}
