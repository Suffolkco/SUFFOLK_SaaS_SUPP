...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyehimslower@suffolkcountyny.gov", emailAddress, "", "Batch Job - DEQ_UPDATE_ALT_ID", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
        ...
        //var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = 'SITE-22-00500'";
        //var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = 'SITE-20-00639'";
        //var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = 'SITE-20-01676'";
        //var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = 'SH23'";
        //var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = 'BR99'";
        //var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = 'FISHKILL'";

		//var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID IN ('SITE-22-00846','SITE-22-00848','SITE-22-00850','SITE-22-00851','SITE-22-00852','SITE-22-00853','SITE-22-00854','SITE-22-00855','SITE-22-00856','SITE-22-00857','SITE-20-00967','SITE-20-00968','SITE-20-02461','SITE-20-01108','SITE-20-01109','SITE-20-01110','SITE-20-01111','SITE-20-01113','SITE-20-01114','SITE-20-01115','SITE-20-01116','SITE-20-01118','SITE-20-01119','SITE-20-01120','SITE-20-01121','SITE-20-01122','SITE-21-01914','SITE-20-01022','SITE-20-01023','SITE-20-01024','SITE-20-01025','SITE-20-01026')";
		
		var from = "SITE-22-040400";
		var updateTo = "SITE-040400";
		'" + dateToCheck + "'		
		var vSQL0 = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = '"+ from + "'";
		var vSQL1 = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = '" + updateTo + "'";
		logDebugLocal(vSQL0);

        var output = "Record ID\n";  		
        
		// SQL to pull active OPC site records that has NO child Tank records		
		var vExistingRecordID = doSQLSelect_local(vSQL0);  	
		var vReplaceTo = doSQLSelect_local(vSQL1);
		
		logDebugLocal("Pulling number of records with matching new alt id:" +  vReplaceTo.length);
		// Only there is no existing custom ID already in the system
		if (vReplaceTo.length == 0)
		{
			logDebugLocal("No existing alt ID found for this record. Pass: " + updateTo);  
		
			// Run the query to pull the current Alt ID			
			logDebugLocal("Pulling the record ID to update alt ID: " + from);  
			logDebugLocal("Update this alt ID record count: " + vExistingRecordID.length);  
			if (vExistingRecordID.length == 0)
			{
				logDebugLocal("No existing record ID found to update. Exit: " + from);  
			}
			else
			{
				logDebugLocal("Try to loop and updating record ID: " + from);  
				for (r in vExistingRecordID)
				{
					recordID = vExistingRecordID[r]["recordNumber"];      
					...
					if (cap)
					{
						...
						if (capmodel.isCompleteCap())
						{
							...
							aa.cap.updateCapAltID(capId, updateTo);
							logDebugLocal("Batch Job " + batchJobName + "Updated record ID from: " + from + " to " + updateTo);  
...
function renewPermit() {
	var itemCap = capId;
	var itemCapIDString = capIDString;

	// 1. in case this cap is already a renewal record, we should work in the parent context
	if (itemCapIDString.indexOf('-REN') > 1) {
		var parentsArr = getParents(appTypeString);
		if (parentsArr && parentsArr.length > 0) {
			itemCap = parentsArr[0];
			itemCapIDString = itemCap.getCustomID();
		} else {
			logDebug(
				'**Error: parent permit cap is not linked to this renewal record.'
			);
			return null;
		}
	}
	var migratedRec = false;
	if (itemCapIDString.indexOf('-') == -1) migratedRec = true;

	// 2. Get next child Seq
	var seqArr = new Array();
	var childrenArr = getChildren(appTypeString, itemCap);
	if (childrenArr)
		for (cIdx in childrenArr) {
			if (
				!migratedRec &&
				childrenArr[cIdx].getCustomID().indexOf(itemCapIDString + '-REN') > -1
			) {
				var seqVal = parseInt(
					childrenArr[cIdx].getCustomID().replace(itemCapIDString + '-REN', '')
				);
				if (!isNaN(seqVal)) seqArr.push(seqVal);
			} else if (
				migratedRec &&
				childrenArr[cIdx].getCustomID().indexOf('-REN') > -1
			) {
				var repTxt = childrenArr[cIdx]
					.getCustomID()
					.substr(
						childrenArr[cIdx].getCustomID(),
						childrenArr[cIdx].getCustomID().indexOf('-REN') + 4
					);
				var seqVal = parseInt(
					childrenArr[cIdx].getCustomID().replace(repTxt, '')
				);
				if (!isNaN(seqVal)) seqArr.push(seqVal);
			}
		}
	...
	// 3. Create Child
	var childCapId = createCap(appTypeString, '');

	// 4. set cap altid
	var childNewAltID = itemCapIDString + '-REN01';
	if (migratedRec) childNewAltID = childCapId.getCustomID() + '-REN01';
	if (seqArr.length > 0) {
		lastSeq =
		...
		if (migratedRec)
			childNewAltID =
				...
				'-REN' +
				(String(lastSeq).length > 1 ? lastSeq : '0' + String(lastSeq));
		else
			childNewAltID =
				itemCapIDString +
				'-REN' +
	...
	aa.cap.updateCapAltID(childCapId, childNewAltID);

	childCapId = aa.cap.getCapID(childNewAltID).getOutput();
}
