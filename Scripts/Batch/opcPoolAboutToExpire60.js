/*------------------------------------------------------------------------------------------------------/
| Program: opcPoolAboutToExpire60.js  Trigger: Batch| 
| 
| This batch script will run daily to send email notification to Pool owner and operator 60 days before
| the expiration 
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
var rtArray = ["DEQ/OPC/Swimming Pool/Permit"];
/*----------------------------------------------------------------------------------------------------/
|
    ...
    try 
    {
        for (var i in rtArray) 
        {
            ...
            for (var j in recArray) 
            {
                ...
                if(appStatus == "Active")
                {
                    logDebug("Application has " +  appStatus + " status");
                        ...
                        if(capmodel.isCompleteCap())
                        {
                            ...
                            if (b1ExpResult.getSuccess())
                            {
                                ...
                                if (curExp != null)
                                {
                                    ...
                                    var convertedDate = (dateLookingFor.getMonth() + 1) + "/" + dateLookingFor.getDate() + "/" + dateLookingFor.getFullYear();
                                    logDebug("60 days before the expiration date: " + convertedDate);
                                    ...
                                    var thirtyDaysPrior = new Date(dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), 30));                                    
                                    appExpDateCon = (thirtyDaysPrior.getMonth() + 1) + "/" + thirtyDaysPrior.getDate() + "/" + thirtyDaysPrior.getFullYear();
                                    logDebug("Application Expiration Date minus 30: " + appExpDateCon);
                                    ...
                                    if (expDateCon == convertedDate)
                                    {
                                        ...
                                        logDebug("Record: " + capIDString + " Status: " + appStatus);
                                        ...
                                        logDebug("Cap name: " + appTypeArray[1]);
                                        
                                        //addParameter(emailParams, "$$capName$$", appTypeArray[1]);
                                        
                                        var fid = getAppSpecific("NYS Facility ID", capId);
                                        logDebug("NYS Facility ID: ", fid);

                                                                           
                                        //var appName = aa.cap.getCap(capId).getOutput().specialText;
                                        ...
                                        for (con in conArray)
                                        {
											...
											conArray[con].contactType == "Management Company")
                                            {                                             
                                                var address1 = conArray[con].addressLine1;				
                                                var city = conArray[con].city;
                                                var state = conArray[con].state;
                                                var zip = conArray[con].zip;											
												//addParameter(emailParams, "$$I$$", altId);
												addParameter(emailParams, "$$p$$", shortNotes);
												//addParameter(emailParams, "$$a$$", address1);
												//addParameter(emailParams, "$$c$$", city);
												//addParameter(emailParams, "$$s$$", state);
												//addParameter(emailParams, "$$z$$", zip);	
                                                addParameter(emailParams, "$$f$$", fid);	
                                                addParameter(emailParams, "$$eD$$", expDateCon);	
                                                addParameter(emailParams, "$$-30$$", appExpDateCon);	
                                                conEmail2 = conArray[con].email;
                                                sendNotification("", conEmail2, "", "DEQ_OPC_POOL_EXPIRE_IN_60_DAYS", emailParams, reportFile);
                                            }							
                                        }	                                                                                
