...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyphp@suffolkcountyny.gov", emailAddress, "", "Batch Job - EH_NEXT_BILLING_DATE_UPDATE", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
        ...
        {		
            ...
            if (cap)
            {
                ...
                if (capmodel.isCompleteCap())
                {
					...
					var billingDate = getAppSpecific("Next Billing Date", capId);   
					
					// FSP Permit - change Change of Owner workflow task status
					if(appTypeArray[2] == "Food Protection" && appTypeArray[3] == "Permit")
					{
						// Check workflow Permit Status to be Change of Owner first
						...
						if (workflowResult.getSuccess())
						{
							...
							for (i in wfObj)
							{
								if (wfObj[i].getTaskDescription() == "Permit Status" && wfObj[i].getActiveFlag().equals("Y") &&
								wfObj[i].getDisposition() == "Change of Owner")
								{
									
									logDebugLocal("Task Status : " + wfObj[i].getDisposition());																								
									logDebugLocal("Original billing date : " + capIDString + " - " + billingDate);										
										
									//logDebugLocal(isBlank(billingDate));

									if(!matches(billingDate,undefined,null,""))
									{	
										editAppSpecific("Next Billing Date", "", capId);
										logDebugLocal("Update Food Permit Next Billing Date: " + capIDString + " to blank.");
										fspCount++;
									}
					...
					else if (appTypeArray[2] == "Mobile" && appTypeArray[3] == "Permit")
					{
						logDebugLocal("Original billing date : " + capIDString + " - " + billingDate);
