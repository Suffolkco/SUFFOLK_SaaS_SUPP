/*------------------------------------------------------------------------------------------------------/
| Program:DCA_DOCKET_VENDOR_LICENSE_OBTAINED.js: Batch
| 
| This batch script will run daily to check if a vendor has applied for license  
| for an existing docket that has waiver on.
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - DCA_DOCKET_VENDOR_LICENSE_OBTAINED", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    ...
    try 
    {
       ...
       var count = 0;
		//TEST only
		var dateToCheck = '08/23/2023';
		// Pull all licenses submitted today
		var vSQL = "SELECT B3.B1_FNAME as ApplicantFirstName, B3.B1_LNAME as ApplicantLastName, B3.B1_BIRTH_DATE as ApplicantBirthDate, B3.B1_BUSINESS_NAME as BusinessName, B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1, b3contact B3 WHERE B1.SERV_PROV_CODE = 'SUFFOLKCO' and B1_PER_GROUP = 'ConsumerAffairs' and B1.B1_PER_TYPE = 'Licenses' and B1_PER_CATEGORY = 'NA' AND b1.serv_prov_code = b3.serv_prov_code and b1.b1_per_id1 = b3.b1_per_id1 and b1.b1_per_id2 = b3.b1_per_id2 and b1.b1_per_id3 = b3.b1_per_id3 and b3.b1_contact_type = 'Applicant' and B1.B1_FILE_DD = '" + dateToCheck + "'";
				
		// Pull all docket with waver
		var vDocketSQL = "select B3.B1_FNAME as VendorFirstName, B3.B1_LNAME as VendorLastName, B3.B1_BIRTH_DATE as VendorBirthDate, B3.B1_BUSINESS_NAME as BusinessName, B1.B1_ALT_ID as recordNumber, bc.B1_CHECKBOX_DESC, bc.B1_CHECKLIST_COMMENT from b1permit b1, b3contact b3, bchckbox bc where b1.serv_prov_code = 'SUFFOLKCO' AND B1.SERV_PROV_CODE = B3.SERV_PROV_CODE AND B1.B1_PER_ID1 = B3.B1_PER_ID1 AND B1.B1_PER_ID2 = B3.B1_PER_ID2 AND B1.B1_PER_ID3 = B3.B1_PER_ID3 and b3.b1_contact_type = 'Vendor' AND B1.SERV_PROV_CODE = Bc.SERV_PROV_CODE AND B1.B1_PER_ID1 = Bc.B1_PER_ID1 AND B1.B1_PER_ID2 = Bc.B1_PER_ID2 AND B1.B1_PER_ID3 = Bc.B1_PER_ID3 and bc.B1_CHECKBOX_TYPE = 'HEARING' and bc.B1_CHECKBOX_DESC = 'Waiver' and BC.B1_CHECKLIST_COMMENT = 'Yes' and B1_PER_GROUP = 'ConsumerAffairs' and B1.B1_PER_TYPE = 'Docket'";

// To do, pull birth date as well from applicant on the license record
// To do, pull birth date from Docket vendor


        var vSQLResult = doSQLSelect_local(vSQL);
		var vDocSQLResult = doSQLSelect_local(vDocketSQL);
		var emailParams = aa.util.newHashtable();

		logDebugLocal("******** There are " + vSQLResult.length + " licenses submitted today*********\n");
		logDebugLocal("******** There are " + vDocSQLResult.length + " dockets with wavier*********\n");

		for (r in vSQLResult)
        {		
            recordID = vSQLResult[r]["recordNumber"];     
            firstName = vSQLResult[r]["ApplicantFirstName"];
			lastName = vSQLResult[r]["ApplicantLastName"];
			dob = vSQLResult[r]["ApplicantBirthDate"];

			logDebugLocal(recordID + ", " + firstName + ", " + lastName + ", " + dob);

			
			for (v in vDocSQLResult)
			{
				vrecordID = vDocSQLResult[v]["recordNumber"];     
				vfirstName = vDocSQLResult[v]["VendorFirstName"];
				vlastName = vDocSQLResult[v]["VendorLastName"];		
				vdob = vDocSQLResult[v]["VendorBirthDate"];
								
				logDebugLocal("Lic Applicant DOB: " + dob + "| Docket Vendor DOB: " + vdob);

				if (matches(vfirstName.toUpperCase(), firstName.toUpperCase()) && 
				matches(vlastName.toUpperCase(), lastName.toUpperCase()))
				{
					logDebugLocal("********************************************************************");
					logDebugLocal("LICENSE #: " + recordID + ", Lic Vendor First Name: " + firstName + ", Lic Vendor Last Name: " + lastName + ", Lic Business Name: " +businessName + ", App DOB: " + dob);
					logDebugLocal("DOCKET #: " + vrecordID + ", Docket First Name: " + vfirstName + ", Docket Last Name:" + vlastName + ", Docket Business Name:" +vbusinessName + ", Docket Vendor DOB: " + vdob);
					logDebugLocal("Found matching first and last name in Docket " + vrecordID + " and License Number: " + recordID);						

					if (dob == vdob)
					{
						logDebugLocal("Lic Applicant DOB: " + dob + "matches with Docket Vendor DOB: " + vdob);
						if (docketNumbers == "")
						{
							docketNumbers = firstName + " " + lastName + " has submitted for license application " + recordID + " today. There is a waiver in docket record with the same first and last name. Please verify: " + vrecordID;
						}
						count++;
					}
					else
					{
						logDebugLocal("But Lic Applicant DOB: " + dob + "DOES NOT match with Docket Vendor DOB: " + vdob);
					}
			...
			addParameter(emailParams, "$$altID$$", docketNumbers);
			//sendNotification("", staffEmailsToSend, "", "DCA_DOCKET_VENDOR_LICENSE_OBTAINED", emailParams, null);
			//sendNotification("", "ada.chan@suffolkcountyny.gov", "", "DCA_DOCKET_VENDOR_LICENSE_OBTAINED", emailParams, null);
			//logDebugLocal("Email was sent.");
			logDebugLocal("********************************************************************");
			logDebugLocal("Total number of license obtained found: " + count);
