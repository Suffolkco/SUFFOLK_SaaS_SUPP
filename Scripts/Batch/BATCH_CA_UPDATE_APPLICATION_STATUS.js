/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_CA_UPDATE_APPLICATION_STATUS.js
| Trigger: Batch
| Client: Suffolk
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "BATCH_CA_UPDATE_APPLICATION_STATUS", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
function mainProcess() 
{
    ...
    {  
        ...
        var vSQL = "SELECT B1.B1_ALT_ID as recordNumber, B1.B1_FILE_DD as openDate FROM B1PERMIT B1 WHERE B1.SERV_PROV_CODE = 'SUFFOLKCO' and B1_PER_GROUP = 'ConsumerAffairs' and B1.B1_PER_TYPE in ('ID Cards' , 'Licenses', 'Registrations') and B1.B1_APPL_STATUS = 'Waiting on Applicant'";
        ...
        for (r in vResult)
        {
            ...
            if (cap)
            {
                ...
                {
                    if (matches(getAppStatus(), "Waiting on Applicant"))
                    {                        
                        ...
                        var expiredDate = convertDate(newDate);
                        ...
                        {
                            //logDebugLocal("Record " + capIDString + " opened on: " + fileDateYYYYMMDD + ". The record has been opened longer than 6 months after record open date: " + newDate);
                            ...
                            if (workflowResult.getSuccess())
                            {
                                ...
                                for (i in wfObj)
                                {
                                    if (wfObj[i].getTaskDescription() == "Test")
                                    {
                                        if (wfObj[i].getDisposition() == "No Show")
                                        {
                                            count++;
                                            logDebugLocal(capIDString + " is @Test and status No Show."); 
                                            aa.cap.updateAppStatus(capId, "Set to Closed-Incomplete from Batch", "Closed-Incomplete", sysDate, "", systemUserObj);                                        
                                            logDebugLocal("Updating app status for " + capIDString + " to Closed-Incomplete");
    
                                            // What do we do with the workflow task and status? Workflow is still open.                                      ;
                                            closeTask("Test", "Closed-Incomplete", "", "");
                                            logDebugLocal("Update workflow status for " + capIDString + " to Closed-Incomplete");                                            
                                        }
                                    }
                                }                             
                        ...
                        {                            
                            logDebugLocal("Today's date is " + sDateMMDDYYYY + ". " + capIDString + " opens less than 6 months. Skip."); 
                            break;                           
                        }
        ...
        logDebugLocal("<b> Total of record Id that has been updated:  " + count);
    }
    catch (err)
    {
