/*------------------------------------------------------------------------------------------------------/
| Program: OPCTankTestingAboutToExpire.js  Trigger: Batch
| Client: SuffolkCo
| Version 1.0 Zachary McVicker 01/16/2019
...
function mainProcess() 
{
	...
	if (matches(curMonth.toString(), "1", "2", "4", "6", "7", "9", "11"))
	{
		ateDate = new Date(dateAdd((startDate.getMonth() + 1) + "/" + "01/" + "/" + startDate.getFullYear(), 31));
		logDebug("Adding 31 days");
	}
	else if (matches(curMonth.toString(), "3", "5", "8", "10"))
	{
		ateDate = new Date(dateAdd((startDate.getMonth() + 1) + "/" + "01/" + "/" + startDate.getFullYear(), 30));
		logDebug("Adding 30 days");
	}
	else
	{
		ateDate = new Date((startDate.getMonth() + 2) + "/" + "01/" + "/" + startDate.getFullYear());
	...
	{	
		...
		{	
			...
			if (cap) 
			{
				...
				if(appTypeArray[0] == "DEQ" && appTypeArray[1] == "OPC" && appTypeArray[2] == "Hazardous Tank" && appTypeArray[3] == "Permit") 
				{
					var capmodel = aa.cap.getCap(capId).getOutput().getCapModel(); 
					var status = capmodel.getCapStatus();
					if (status == "Active")
					{
                        var epaREgulatedTank = getAppSpecific("EPA Tank", capId);
                        logDebug("epaREgulatedTank: " + epaREgulatedTank);
                        if(epaREgulatedTank == "Yes")
                        {
						...
						var setName = "Tank Testing Renewals" + " " + month + "/" + year;
						...
						for (con in conArray)
						{
							if (conArray[con].contactType == "Tank Owner" || conArray[con].contactType == "Billing Contact")
							{
								...
								//addParameter(params, "$$expDate$$", b1ExpDate);
								if (conEmail != null)
								{
									sendNotification("", conEmail, "", "OPC_TANK_TESTING_RENEWAL_NOTIFICATION", params, reportFile);
								}
							}
						}
					}
