/*------------------------------------------------------------------------------------------------------/
| Program: garbageAboutToExpire30.js  Trigger: Batch
| Client: JCCAL
...
if (paramsOK) 
{
	...
	if (!timeExpired) 
	{
		...
		aa.sendMail("noreplyehims@suffolkcountyny.gov", emailAddress, "", "Batch Job - garbageAboutToExpire30", emailText);
	}
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
function mainProcess() 
{
    ...
    {
		var total = 0;
        logDebug("We are in an odd year, batch script garbageAboutToExpire30 will run");
        try 
        {
            for (var i in rtArray) 
            {
                ...
                for (var j in recArray) 
                {
					...
					logDebug("Looping through " + capIDString);
					cap = aa.cap.getCap(capId).getOutput();	
					var taskCheck = false;

                    if (cap)
                    {
						if (getAppStatus() != "Renewal Submitted")
						{
							...
							for (i in wfObj) 
							{
								...
								if (fTask.getTaskDescription() == "Application Review" && fTask.getDisposition() == "Application Complete")
								{
									//aa.print("We're getting the task descriptions and the status is " + fTask.getDisposition());
									taskCheck = true;
									//aa.print(taskCheck);
									//break;
								}
										...
										if (curExp != null)
										{
											...
											if (curSt != null)
											{
												...
												{	
													...
													{
														//aa.print("We are checking if " + curExpCon + " is equal to " + "12/31" + "/" + startDate.getFullYear())
														//aa.print("<b>" + capIDString + "</b>");
														...
														if (conEmail != null)
														{
															...
															sendNotification("", conEmail, "", "DEQ_WWM_GARBAGE_ABOUT_TO_EXPIRE_30", emailParams, null);
															total++;
														}
    ...
    { 
        logDebug("We are in an even year, batch script will update 0 records");
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========END=Main=Loop================>
