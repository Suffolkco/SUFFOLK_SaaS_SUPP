/*------------------------------------------------------------------------------------------------------/
| Program: wwmApplicationAboutToExpire30-60.js  Trigger: Batch
| Client: JCCAL
| Version 1.0 Casey Gray 01/16/2019
...
var br = "<BR>";
var emailAddress = "casey.gray@scubeenterprise.com";//email to send report
...
var rtArray2 = ["DEQ/WWM/Commercial/Application", "DEQ/WWM/Residence/Application"];
/*----------------------------------------------------------------------------------------------------/
|
...
if (paramsOK) 
{
	...
	if (!timeExpired) 
	{
		...
		aa.sendMail("noreplyehims@suffolkcountyny.gov", emailAddress, "", "Batch Job - wwmExpiration", emailText);
	}
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
            ...
            for (var j in recArray) 
            {
                ...
                if (cap)
                {
					...
					for (i in wfObj)
					{
						...
						if (fTask.getTaskDescription() != null && fTask.getTaskDescription() == ("Plans Coordination") && fTask.getDisposition() != null && fTask.getDisposition() == ("Approved"))
						{
							appCheck = true;
							//logDebug("Skipping this record");
						}	
					}
					//logDebug("Appcheck is: " + appCheck);
                    var capmodel = aa.cap.getCap(capId).getOutput().getCapModel();
                    if(capmodel.isCompleteCap() && appCheck == false)
                    {
                        ...
                        if (b1ExpResult.getSuccess())
                        {
                            ...
                            if (curExp != null)
                            {
                                ...
                                //logDebug("Current exp is: " + curExpCon);
                                var startCon = (startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear();
                                //logDebug("Start Con is: " + startCon);
                                var curSt = b1Exp.getExpStatus();
                                if (curSt != null)
                                {
									dateDif = parseFloat(dateDiff(startCon, curExpCon));
									dateDifRound = Math.floor(dateDif);
									//logDebug("dateDifRound is: " + dateDifRound);
									if (dateDifRound == 60)
									{
										//b1Exp.setExpStatus("About to Expire");
										//aa.expiration.editB1Expiration(b1Exp.getB1Expiration());
										logDebug("<b>" + capIDString + "</b>" + " will expire in 60 days, sending notification");
										...
										for (con in conArray)
										{
											conEmail.push(conArray[con].email);
										}
										if (conEmail != null)
										{
											for (x in conEmail)
											{
												sendNotification("", conEmail[x], "", "DEQ_WWM_ABOUT_TO_EXPIRE_60", emailParams, null);
											}
										}
									}
									if (dateDifRound == 30)
									{
										//b1Exp.setExpStatus("About to Expire");
										//aa.expiration.editB1Expiration(b1Exp.getB1Expiration());
										logDebug("<b>" + capIDString + "</b>" + " will expire in 30 days, sending notification");
										var emailParams = aa.util.newHashtable();
										var conArray = getContactArray();
										var conEmail = [];
										var altId = capId.getCustomID();
										...
										if (conEmail != null)
										{
											for (x in conEmail)
											{
												sendNotification("", conEmail[x], "", "DEQ_WWM_ABOUT_TO_EXPIRE_30", emailParams, null);
											}
										}
									}        
                                }
...
function processComRes() 
{
    try 
    {
        for (var i in rtArray2) 
        {
            var thisType = rtArray2[i];
            var capModel = aa.cap.getCapModel().getOutput();
            var appTypeArray = thisType.split("/");
            // Specify the record type to query
            capTypeModel = capModel.getCapType();
            ...
            var recArray = recordListResult.getOutput();
            logDebug("<b>" + thisType + " after Preliminary Approval" + "</b>");
            ...
            for (var j in recArray)
            {
                ...
                if (cap)
                {
                    ...
                    var capmodel = aa.cap.getCap(capId).getOutput().getCapModel();
                    if(capmodel.isCompleteCap() && appCheck == true)
                    {
                        b1ExpResult = aa.expiration.getLicensesByCapID(capId);
                        if (b1ExpResult.getSuccess())
                        {
                            ...
                            if (curExp != null)
                            {
                                ...
                                if (curSt != null)
                                {
									...
									if (dateDifRound == 60)
									{
										b1Exp.setExpStatus("About to Expire");
										aa.expiration.editB1Expiration(b1Exp.getB1Expiration());
										logDebug("<b>" + capIDString + "</b>" + " will expire in 60 days, sending notification and renewal info has been set to About to Expire");
										var emailParams = aa.util.newHashtable();
										var conArray = getContactArray();
										var conEmail = [];
										var altId = capId.getCustomID();
									...
									if (dateDifRound == 30)
									{
										renCheck = getRenewalCapByParentCapIDForReview(capId);
										//logDebug("Renewal check is: " + renCheck);
										var emailParams = aa.util.newHashtable();
										var conArray = getContactArray();
										var conEmail = [];
										var altId = capId.getCustomID();
										...
										if (renCheck == null)
										{
											b1Exp.setExpStatus("About to Expire");
											aa.expiration.editB1Expiration(b1Exp.getB1Expiration());
											logDebug("<b>" + capIDString + "</b>" + " will expire in 30 days, sending notification and renewal info has been set to About to Expire");
											for (con in conArray)
											{
												conEmail.push(conArray[con].email);
											}
										...
										else
										{
											...
											aa.expiration.editB1Expiration(b1Exp.getB1Expiration());
											logDebug("<b>" + capIDString + "</b>" + " will expire in 30 days and has already been renewed once, sending notification and setting the expiration status to 'Expired' disabling further renewals");
											for (con in conArray)
											{
												conEmail.push(conArray[con].email);
											}
