/*------------------------------------------------------------------------------------------------------/
| Program: SwimPoolBiannualFeeOdd.js  Trigger: Batch
| Version 1.0 Casey Gray 12/11/2018
| This batch script will be scheduled to run once every year on 7/15 (60 days before expiration), it will only execute on odd years.
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
	...
	try 
	{
		for (var i in rtArray) 
		{
			...
			for (var j in recArray) 
			{
				...
				var outdoorCheck = false;
				if (cap)
				{
					var poolTable = loadASITable("SWIMMING POOL INFORMATION");
					for (p in poolTable)
					{
						if (poolTable[p]["Location"] == "Outdoor")
						{
							outdoorCheck = true;
							//logDebug("We found an outdoor pool");
							break;
						}
					}
					...
					if(capmodel.isCompleteCap())
					{
						...
						{
							fTask = wfObj[i];
							if (fTask.getTaskDescription() != null && fTask.getTaskDescription().equals("Final Review") && fTask.getDisposition() != null && fTask.getDisposition().equals("Approved"))
							{
								statCheck = true;
							}	
						}
						if (statCheck == true && outdoorCheck == true)
						{
							...
							for (x in conArray)
							{
								if (conArray[x].contactType == "Property Owner")
								{
									propertyOwner = conArray[x];
									propEmail = propertyOwner.email;
									sendNotification("", propEmail, "", "DEQ_OPC_SWIMMING_POOL_BIANNUAL_FEE", emailParams, null);
								}
							}
						}
					}
...
{
	logDebug("Is is an even year, not processing indoor pools");
}
