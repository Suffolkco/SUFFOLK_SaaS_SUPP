/*------------------------------------------------------------------------------------------------------/
| Program: DEQ_WWM_INSTALLER_LP.js  Trigger: Batch|  
    ...
    {       
        for (var i in rtArray) 
        {
            ...
            for (var j in recArray) 
            {
                ...
                //EHIMS-4494: Jen does not want this to be restricted to "Active"
                //if(capIDString == "LW-36752")
                //if(appStatus == "Active")
				{
					if (cap)
					{         
                        createUpdateRefLicProfWWM(capId);					
                        count++;
				    }			
                }                                 

            }     
            logDebug("Total Active Liquid Waste Records: " + count);
            logDebug("Total update to WWM Installer LPs: " + updateCount);
            logDebug("Total count to add the latest number in the business license #: " + totalAddCount);
        }
...
function createUpdateRefLicProfWWM(capId) {

    var licenType = "WWM Installer";
    if (capId == null) {
                        ...
                        if (capContResult.getSuccess()) 
                        {
                            ...
                            for (con in conArray)
                            {
                                ...
                                /* logDebug(capIDString + ":" + licenType + ":First Name: " +
                                    ...
                                    //logDebug("*** Look for: ***" + capIDString+ ". License Type: " + licenType);
                                    ...
                                    {                                                   
                                        ...
                                        //logDebug("6. Found existing WWM Installer LP with license number: " + licStateNum);
                                    }
                                    else
                                    {
                                        //logDebug("Does not find existing WWM Installer LP for: " + capIDString);
                                    }
                                    ...
                                    {
                                        logDebug(capIDString + ", Name: " + conArray[con].getCapContactModel().getPeople().getFirstName() + "," +  conArray[con].getCapContactModel().getPeople().getLastName());
                                        //*createUpdateRefLicProfWWMInstaller(capId, false, licenType);            
                                        updateCount++;
                                    ...
                                    {                       
                                        logDebug("8. Update state license " + capIDString  + " business license number custom field to: " + licStateNum);                                    
...
function createUpdateRefLicProfWWMInstaller(capId, relate, rlpType) {
	if (capId == null) {
		logDebug("createUpdateRefLicProfWWMInstaller : capId object is null");
		return;
	}
