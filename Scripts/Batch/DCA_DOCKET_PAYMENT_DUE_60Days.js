/*------------------------------------------------------------------------------------------------------/
| Program:DCA_DOCKET_PAYMENT_DUE_60Days.js: Batch
| 
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - DCA_DOCKET_PAYMENT_DUE_60Days", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
		...
		// Pull Docket Type with payment due date for 60 days
        ...
        {		
            ...
            paymentDueDate = vSQLResult[r]["PayDueDate"];
            ...
            if (cap)
            {
                ...
                {								
					var accountClerkEmail = lookup("DCA_Docket_Email_List", "AccountClerks");   	
					...
					if (workflowResult.getSuccess())
					{
						...
						for (i in wfObj)
						{
							if (wfObj[i].getTaskDescription() == "Payment")
							{
								...
								// Remove existing notice and add LOCK condition
								var conditionName = "Lock"; // Need Greg to confirm the condition since 0-30 days is past due violation as well
								
								var capConditions = getCapConditionByCapID(capId);
								if (capConditions != null)
								{
									//getStandardConditionNumber
									logDebug("Found Condition." + capConditions.length);
									// First check if there is already condition, if there is, do not add.\
									for (loopk in capConditions)
									{
										sourceCapCondition = capConditions[loopk];
										aa.capCondition.deleteCapCondition(capId, sourceCapCondition.getConditionNumber());
										logDebug("Delete Condition." + sourceCapCondition.getConditionDescription());
									
									}
								}
																	
								// Add condition in docket
								addStdCondition("DCA", conditionName, capId);
								// Add Condition in license
								if (licenseCapId)
								{
