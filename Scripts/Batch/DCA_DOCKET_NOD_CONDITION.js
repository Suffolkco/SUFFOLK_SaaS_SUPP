/*------------------------------------------------------------------------------------------------------/
| Program:DCA_DOCKET_NOD_CONDITION.js: Batch
| 
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - DCA_DOCKET_NOD_CONDITION", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
		...
		// Pull Docket Type 
		var vSQL = "SELECT B1.B1_ALT_ID as recordNumber, BC.B1_CHECKLIST_COMMENT as licenseNumber FROM B1PERMIT B1 INNER JOIN BCHCKBOX BC on b1.serv_prov_code = bc.serv_prov_code and b1.b1_per_id1 = bc.b1_per_id1 and b1.b1_per_id2 = bc.b1_per_id2 and b1.b1_per_id3 = bc.b1_per_id3 and bc.B1_CHECKBOX_DESC = 'License Number' and BC.B1_CHECKLIST_COMMENT <> null WHERE B1.SERV_PROV_CODE = 'SUFFOLKCO' and B1_PER_GROUP = 'ConsumerAffairs' and B1.B1_PER_TYPE = 'DOCKET' and B1_PER_CATEGORY = 'NA' ";
		
		
        var vSQLResult = doSQLSelect_local(vSQL);
		var emailParams = aa.util.newHashtable();

		logDebugLocal("******** Docket Type has : " + vSQLResult.length + "*********\n");
		for (r in vSQLResult)
        {		
            recordID = vSQLResult[r]["recordNumber"];     
            licenseNumber = vSQLResult[r]["licenseNumber"];
            ...
            if (cap)
            {
                ...
                {
					appStatus = getAppStatus(licenseNumber).toUpperCase();
					
					if (matches(appStatus, "ACTIVE", "LICENSE ACTIVE", "PENDING RENEWAL"))
					{
					...
					else
					{
						...
						if (workflowResult.getSuccess())
						{
							...
							for (i in wfObj)
							{
								if (wfObj[i].getTaskDescription() == "NOD")
								{									

									// Add WOS Condition
									addStdCondition("DCA", "Wall of Shame");									
									logDebugLocal("WOS condition added for: " + capIDString + " since license number " + licenseNumber + " has a status of: " + appStatus);
								}
