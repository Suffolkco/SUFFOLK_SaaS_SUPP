        ...
        if (fieldNum.indexOf("-L-BW-ZWR") > 0) {
            ...
            if (siteId) {
                ...
                if (resId) {
                    ...
                    var resId = createChild4LIMS("DEQ", "General", "Lab", "Results", "LIMS Integration " + sysDateMMDDYYYY, siteId);
                    if (resId) {
...
function makeTable(labResults, resId, inspEmail) {
    ...
    */
    ...
    for (var row in labResults) {
        ...
        t["Trace Results"] = "";
        if (r[6] == "INVALID" || r[6] == "Invalid" || r[6] == "LA" || r[6] == "NR") {
            sampleError = true;
        }
        rawTextResult = "" + r[6];
     ...
     //   else {    // 1016 per Josh
            ...
            if (analyteName == "Gross Alpha E" || analyteName == "Gross Alpha P" || analyteName == "Gross Beta" || analyteName == "Tritium") {
                if (textResult == "CONC") {
...
function processCombos(resId) { 
    ...
    for (var rIndex in labResults) {
        ...
        if (prettyName == "Iron (Fe)" || prettyName == "Manganese (Mn)") {
            metalCombos.push(row);
        }
        if (prettyName == "Aldicarb" || prettyName == "Aldicarb-Sulfoxide" || prettyName == "Aldicarb-Sulfone") {
            aldiCombos.push(row);
        }
        if (prettyName == "Simazine" || prettyName == "Atrazine") {
            svocCombos.push(row);
        }
    }
