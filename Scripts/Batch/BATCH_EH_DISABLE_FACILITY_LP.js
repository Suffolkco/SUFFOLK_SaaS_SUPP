...
/*------------------------------------------------------------------------------------------------------/
| Program:BATCH_EH_DISABLE_FACILITY_LP.js: Batch
| 
...
{    
    ...
    {
        var disabledCnt = 0;
        var alreadyDisabledCnt = 0;

        logDebug("Batch script will run"); 
		var vSQL0 = "select DISTINCT B1_LICENSE_NBR as lp from B3CONTRA where B1_LICENSE_TYPE = 'Food Facility' and B1_LICENSE_NBR like 'FA%'";	
        // Test
        //var vSQL0 = "select DISTINCT B1_LICENSE_NBR as lp from B3CONTRA where B1_LICENSE_TYPE = 'Food Facility' and B1_LICENSE_NBR like 'FA0000270'";		

		logDebugLocal(vSQL0);
		// SQL to pull active OPC site records that has NO child Tank records		
		var recordIDs = doSQLSelect_local(vSQL0);  
		logDebugLocal("Pulling number of records with matching FA LP Ids:" +  recordIDs.length);
	
        licenType = "Food Facility";
        for (r in recordIDs)
        {
            lpID = recordIDs[r]["lp"];      
            logDebugLocal("Looking at LP: " + lpID);           

            //Deactivating license profesionals
            var refLic = getRefLicenseProf(lpID) // Load the reference License Professional
        ...
        logDebugLocal("Total disabled count: " + disabledCnt);
        logDebugLocal("LPs that are already enabled and didn't update: " + alreadyDisabledCnt);
