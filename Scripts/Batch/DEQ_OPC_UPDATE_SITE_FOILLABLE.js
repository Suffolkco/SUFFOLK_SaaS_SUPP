/*------------------------------------------------------------------------------------------------------/
| Program:DEQ_OPC_UPDATE_SITE_FOILLABLE: Batch
| 
| This batch script will run one time to All OPC SITE records 
| to set "Restricted Facitlies" custom field based on other field values
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyehimslower@suffolkcountyny.gov", emailAddress, "", "Batch Job - DEQ_OPC_UDPATE_SITE_FOILABLE", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
        ...
        {			
            ...
            if (cap)
            {
                ...
                if (capmodel.isCompleteCap())
                {
					...
					{
						logDebugLocal("Storage Historical Terminal");
					...
					{
						logDebugLocal("Chemical Distributor");
...
/------------------------------------------------------------------------------------------------------*/
function copyCustomField(name, capId)
{
	editAppSpecific("Restricted Facilities", name, capId);
}
