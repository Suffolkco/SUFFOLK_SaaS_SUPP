/*------------------------------------------------------------------------------------------------------/
| Program:DEQ_OPC_UPDATE_TANK_FIELDS.js: Batch
| 
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyehimslower@suffolkcountyny.gov", emailAddress, "", "Batch Job - DEQ_OPC_UPDATE_TANK_FIELDS", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
		...
		// SQL to pull all OPC Tank Permits		
		var vTankSQL = "SELECT DISTINCT B.B1_ALT_ID as recordNumber FROM B1PERMIT B JOIN BCHCKBOX C ON B.B1_PER_ID1 = C.B1_PER_ID1 AND B.B1_PER_ID2 = C.B1_PER_ID2 AND B.B1_PER_ID3 = C.B1_PER_ID3 WHERE B.SERV_PROV_CODE = 'SUFFOLKCO' AND B.B1_PER_GROUP = 'DEQ' AND B.B1_PER_TYPE = 'OPC' AND B.B1_PER_SUB_TYPE = 'Hazardous Tank' AND B.B1_PER_CATEGORY = 'Permit' AND C.B1_CHECKBOX_DESC = 'Official Use Code' AND C.B1_CHECKLIST_COMMENT = 'UR'";       
		...
		logDebugLocal("********OPC Tank Permits records: " + vTankSQLResult.length + "*********\n");
        ...
        {		
            ...
            if (cap)
            {
                ...
                {
					var offUseCode = getAppSpecific("Official Use Code", capId);
				
					if (offUseCode != null && offUseCode == "UR")					
					{		
						logDebugLocal("******* Checking " + capIDString);
						tankStatus = getAppSpecific("Status", capId);
						if (tankStatus != null)
						{
							logDebugLocal("Current Status value: " + tankStatus );
						}
						if (tankStatus != "99-Other")
						{
							editAppSpecific("Status", "99-Other", capId);
							logDebugLocal("Update from offUseCode: " + tankStatus + " to " + "99-Other");
							statusCount++;
							statusTank = statusTank + "," + capIDString;
						}
						else
						{
							logDebugLocal("No update has been made for status: " + capIDString);
						}
					 	otherStatus = getAppSpecific("Other Status", capId);
						 if (otherStatus != null)
						{
							logDebugLocal("Current Other Status value: " + otherStatus);
						}

						
						if (otherStatus != "Unregistered")
						{
							editAppSpecific("Other Status", "Unregistered", capId);										
							logDebugLocal("Update from Other Status: " + otherStatus + " to " + "Unregistered");
							otherStatusCount++;
							otherStatusTank = otherStatusTank + "," + capIDString;
						}
						else
						{
							logDebugLocal("No update has been made for Other Status: " + capIDString);
						}
		...
		logDebugLocal("***********************************************:");
		logDebugLocal("Toal number of tank records with UR are: " + count);
		//logDebugLocal("These are the tank IDs with UR: " + tanks);
		logDebugLocal("***********************************************:");
		logDebugLocal("Number of tank records with Status updated are: " + statusCount);
		logDebugLocal("These are the tank IDs with Status updated: " + statusTank);
		logDebugLocal("***********************************************:");
		logDebugLocal("Number of tank records with Other Status updated are: " + otherStatusCount);
		logDebugLocal("These are the tank IDs with Other Status updated: " + otherStatusTank);
