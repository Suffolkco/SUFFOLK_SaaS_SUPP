/*------------------------------------------------------------------------------------------------------/
| Program: SwimPoolBiannualFeeNotice.js  Trigger: Batch
| Version 1.0 Casey Gray 12/13/2018
...
//logDebug("Date now is: " + nowDate);
//var stMinus = (nowDate - 86400000);
//var stMinusToObj = new Date(stMinus);
...
var rtArray = ["DEQ/OPC/Swimming Pool/Application"];
/*----------------------------------------------------------------------------------------------------/
|
    ...
    for (var i in rtArray) 
    {
        ...
        for (var j in recArray) 
        {
            ...
            if (cap)
            {
				...
				var stMinus = dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), -1);
                ...
                if(capmodel.isCompleteCap())
                {
                    ...
                    if (b1ExpResult.getSuccess())
                    {
                        ...
                        var expStatus = b1Exp.getExpStatus();
                        //logDebug("expiration status is: " + expStatus);
                        if (expStatus == "About to Expire")
                        {
                            var feeExist = false;
                            if (feeExists("SP-BI-01") || feeExists("SP-BI-02") || feeExists("SP-BI-03") || feeExists("SP-BI-04"))
                            {
                                feeExist = true;
                                logDebug("feeExists is: " + feeExist);
                            }
                            var balDue = false;
                            var totalFeeAmount = 0;
                            var fee = 0;
                            var assessedFees = aa.fee.getFeeItems(capId);
                            var feeDateArray = new Array();
                            if(assessedFees.getSuccess())
                            {
                                ...
                                for (ff in feeObjArr)
                                {
                                    ...
                                    if(!matches(fee.getFeeitemStatus(), "VOIDED", "CREDITED", "NEW"))
                                    {
										...
										feeDate = fee.getApplyDate().getEpochMilliseconds();
										//logDebug("Fee date milliseconds is: " + feeDate);
										var feeToObj = new Date(feeDate);
										var feeCon = (feeToObj.getMonth() + 1) + "/" + feeToObj.getDate() + "/" + feeToObj.getFullYear();
                                        feeDateArray.push(feeCon);
                                        //logDebug("Fee Date is: " + feeCon + " pushing fee into our array");
                                    }
                                }
                                if (totalFeeAmount > 0)
                                {
                                //logDebug("Total Amount: " + totalFeeAmount);
                                balDue = true;
                                logDebug("balanceDue is: " + balDue);
                                }
                                dateCheck = false;
                                for (f in feeDateArray)
                                {
									logDebug("We are now comparing: " + feeDateArray[f] + " to " + stMinus);
                                    if (feeDateArray[f] == stMinus)
                                    {
                                        dateCheck = true;
                                        logDebug("dateCheck is: " + dateCheck);
                                    }
                                }
                            }
                            if (feeExist == true && balDue == true && dateCheck == true) //If unpaid biannual fees exists and are > 30 days old
                            {
                                //aa.expiration.editB1Expiration(b1Exp.getB1Expiration());
                                //logDebug("<b>" + capIDString + "</b>");
                                logDebug("Unpaid biannual fees exist and are 1 day old");
                                ...
                                for (con in conArray)
                                {
                                    if (conArray[con].contactType == "Operator" || conArray[con].contactType == "Property Owner")
                                    {
                                        applicant = conArray[con];
                                        conEmail = applicant.email;
                                        if (conEmail != null)
                                        {
                                            sendNotification("", conEmail, "", "DEQ_OPC_SWIMMING_POOL_BIANNUAL_FEE_NOTICE", params, reportFile);
                                        }
                                    }
                                }
                            }
