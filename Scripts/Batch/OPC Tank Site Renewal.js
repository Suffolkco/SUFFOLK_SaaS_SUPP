/*------------------------------------------------------------------------------------------------------/
| Program: OPC Tank Site Renewal.js  Trigger: Batch
                            ...
                            if (b1ExpResult.getSuccess()) 
                            {
                                ...
                                try
                                {
                                    ...
                                    if (expDateCon == convertedDate)
                                    {
                                        ...
                                        var acaURL = "aca.suffolkcountyny.gov/CitizenAccess/Cap";
                                        ...
                                        for (x in conArray)
                                        {
                                            if (conArray[x].contactType == "Tank Owner")
                                            {
                                                ...
                                                sendNotification("", conEmail, "", "OPC_SITE_RENEWAL", emailParams, null);
                                            }
                                        ...
                                        if (childArray)
                                        {
                                            if (childArray.length > 0)
                                            {
                                                for (yy in childArray)
                                                {
                                                    ...
                                                    logDebug("We found this child tank: " + childCapId.getCustomID() + " with this status: "+ childCapStatus);
                                                    if(childCapStatus == "Active")
                                                    {
                                                        var feeExempt = getAppSpecific("Fee Exempt", childCapId);
                                                        if(feeExempt != "Yes" || ownerTypeCheck == true)
                                                        {
                                                            logDebug("Fee Exempt?: " + feeExempt);
                                                            //Commented out: i don't think we need to include these in here.
                                                            //addToSet(childCapId,  "OPC Site Renewals", "OPC Site Renewals");
                                                            var conArray = getContactArray(childCapId);
                                                            var altID = childCapId.getCustomID();
                                                            var emailParams = aa.util.newHashtable();
                                                            var acaURL2 = "https://aca.suffolkcountyny.gov/CitizenAccess/Cap";

                                                            addParameter(emailParams, "$$ALTID$$", altID);
                                                            acaURL2+= getACAUrl(capid);
                                                            addParameter(emailParams, "$$ACAURL", acaURL2);
