/*------------------------------------------------------------------------------------------------------/
| Program: opcTankAboutToExpire.js  Trigger: Batch
| Client: SuffolkCo
| Version 1.0 Casey Gray 12/03/2018
| This batch script will run monthly.
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
if (paramsOK) 
{
	...
	if (!timeExpired) 
	{
		...
		aa.sendMail("noreplyehims@suffolkcountyny.gov", emailAddress, "", "Batch Job - OPC Tank Application About to Expire", emailText);
	}
}
/*------------------------------------------------------------------------------------------------------/
| <===========END=Main=Loop================>
...
function mainProcess() 
{
	...
	//logDebug("Current month is: " + curMonth);
	if (matches(curMonth.toString(), "0", "2", "4", "6", "7", "9", "11"))
	{
		ateDate = new Date(dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), 31));
		//logDebug("Adding 31 days");
	}
	else if (matches(curMonth.toString(), "3", "5", "8", "10"))
	{
		ateDate = new Date(dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), 30));
		//logDebug("Adding 30 days");
	}
	else
	{
		ateDate = new Date(dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), 28));
		//logDebug("Adding 28 days");
	}

	fromDate = (ateDate.getMonth() + 1) + "/" + ateDate.getDate() + "/" + ateDate.getFullYear();
	toDate = (ateDate.getMonth() + 1) + "/" + (ateDate.getDate() + 30) + "/" + ateDate.getFullYear();
	statusCheck = "Pending";
	statusSet = "About to Expire";
	workAppStatus = "About To Expire";
	processLicenses();
}

function processLicenses() 
{
	logDebug("Set currently " + statusCheck + " applications to " + statusSet + " for date range " + fromDate + " to " + toDate + "." + br);
	...
	{	
		...
		{	
			...
			if (cap) 
			{
				...
				if(appTypeArray[0] == "DEQ" && appTypeArray[1] == "OPC" && appTypeArray[2] == "Hazardous Tank" && appTypeArray[3] == "Application") 
				{
					var capmodel = aa.cap.getCap(capId).getOutput().getCapModel(); 
					var status = capmodel.getCapStatus();
					if (status != "Complete")
					{
						starDate.setMonth(starDate.getMonth() + 2); //Adding 1 month to the start date
						var month = starDate.getMonth();
						//logDebug("Month is: " + month);
						var year = (starDate.getYear()) + 1900;
						logDebug("<b>" + altId + "</b>");
						//logDebug("Current expiration is: "+ b1ExpDate);
						b1Exp.setExpStatus(statusSet);
						logDebug("Expiration status updated to 'About to Expire'");
						aa.expiration.editB1Expiration(b1Exp.getB1Expiration());
						var setName = "Tank Installation Renewals" + " " + month + "/" + year;
						addRecordToSet(capId, setName, setName);
						...
						for (con in conArray)
						{
							if (conArray[con].contactType == "Tank Owner" || conArray[con].contactType == "Billing Contact")
							{
								...
								addParameter(emailParams, "$$expireDate$$", b1ExpDate);	

								if (conEmail != null)
								{
									sendNotification("", conEmail, "", "DEQ_OPC_HAZARDOUS TANK_RENEWAL", params, reportFile);
								}
							}
						}
					}
...
function sendNotification(emailFrom, emailTo, emailCC, templateName, params, reportFile)
{
	...
	if(result.getSuccess())
	{
		logDebug("Sent email to Tank Owner/Billing Contact successfully!");
		return true;
	}
	else
	{
