/*------------------------------------------------------------------------------------------------------/
| Program:DEQ_WWM_UPDATE_PARCELS.js: Batch
| 
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyehimslower@suffolkcountyny.gov", emailAddress, "", "Batch Job - DEQ_WWM_UPDATE_PARCELS", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
        ...
        // all
		var vSQL = "select B.B1_ALT_ID as recordNumber,C.B1_CHECKLIST_COMMENT as multiId FROM B1PERMIT B JOIN BCHCKBOX C ON B.B1_PER_ID1 = C.B1_PER_ID1 AND B.B1_PER_ID2 = C.B1_PER_ID2 AND B.B1_PER_ID3 = C.B1_PER_ID3 WHERE C.B1_CHECKBOX_DESC = 'Tax Map Numbers' AND C.B1_CHECKLIST_COMMENT <> '' AND (B.B1_APPL_CLASS = 'Complete' OR  B.B1_APPL_CLASS is null) AND B.B1_PER_GROUP = 'DEQ' AND B.B1_PER_TYPE = 'WWM' AND B.B1_PER_CATEGORY = 'Application'";
		// Test single record
		//var vSQL = "select B.B1_ALT_ID as recordNumber,C.B1_CHECKLIST_COMMENT as multiId FROM B1PERMIT B JOIN BCHCKBOX C ON B.B1_PER_ID1 = C.B1_PER_ID1 AND B.B1_PER_ID2 = C.B1_PER_ID2 AND B.B1_PER_ID3 = C.B1_PER_ID3 WHERE C.B1_CHECKBOX_DESC = 'Tax Map Numbers' AND C.B1_CHECKLIST_COMMENT <> '' AND (B.B1_APPL_CLASS = 'Complete' OR  B.B1_APPL_CLASS is null) AND B.B1_PER_GROUP = 'DEQ' AND B.B1_PER_TYPE = 'WWM' AND B.B1_PER_CATEGORY = 'Application'"// AND B.B1_ALT_ID = 'R-20-1781'";//'R-20-1144'";
        var output = "Record ID\n";  		
        var alreadyExistCnt = 0;
		var listOfFailedRecordIds ="";
		var listofExistedRecordIds = "";
		var vResult = doSQLSelect_local(vSQL);  	
		var totalParcelsAdded = 0;
		var failedToAdd = 0;
		logDebugLocal("Total number of records with multiple tax map numbers: " +  vResult.length);	
		for (r in vResult)
        {
			logDebugLocal("******************************************************************************");
            recordID = vResult[r]["recordNumber"];      
			taxIDs = vResult[r]["multiId"];    

			            //output += recordID + "\n";
            capId = getApplication(recordID);
            capIDString = capId.getCustomID();

			logDebugLocal("List of custom field tax numbers for " + capIDString  + " detected: \n" + taxIDs);
            ...
            if (cap)
            {
                ...
                if (capmodel.isCompleteCap())
                {
					...
					if (capParcelResult.getSuccess())
					{
						...
						//var taxIdSplit = taxIDs.split(" ");
						//logDebugLocal("taxIdSplit:" +  taxIdSplit);
											
						// split one or more commas or newline characters
						//const re = "/\s*[,\r\n]+\s*/";
						// split carriage return/New line
						const re = /\r?\n/;
						var taxIdSplit = taxIDs.split(re);
						
						for (id in taxIdSplit)
						{
							tid = taxIdSplit[id];
							var taxId = new String(tid);
							noSpaceParcelNo = taxId.replace(/\s/g, '');			
							var taxIdLength = noSpaceParcelNo.length;
							//logDebugLocal("Tax Map Number length: " +  taxIdLength);
							var exist = false;
							if (taxIdLength != 19)  
							{
								logDebugLocal("Unable to add parcel number " + taxId + " to " + capIDString+ " since it is " + taxIdLength + " digits.");
								if (failedToAdd == 0)
								{
									listOfFailedRecordIds = capIDString;
								}
								else
								{
									listOfFailedRecordIds = listOfFailedRecordIds + ", " + capIDString;
								}
								failedToAdd++;
							}
							else
							{
								for (zz in Parcels)
								{
									...
									logDebugLocal("Compare existing Parcel Number in record " + capIDString + ": " + parcelNumber);
									...
									if (parcelNumber != null)
									{
										...
										if (noSpaceParcelNo == parcelTxt)
										{
											exist = true;
											logDebugLocal("Parcel Number exists in record " + capIDString + " already. Skip adding " + parcelNumber);
											if (alreadyExistCnt == 0)
											{
												listofExistedRecordIds = capIDString ;
											}
											else
											{
												listofExistedRecordIds = listofExistedRecordIds + ", " + capIDString;
											}
								...
								if (!exist)
								{
									...
									if (capParcelResult.getSuccess())
									{
										...
										//for (zz in Parcels)
										{
											...
											newCapParcel.setL1ParcelNo(taxId);
											newCapParcel.setParcelNo(taxId);
											var result =aa.parcel.createCapParcel(newCapParcel);
											if (result.getSuccess())
											{
												totalParcelsAdded++;
												logDebugLocal(totalParcelsAdded + ". Added parcel number successfully: " + taxId + " to " + capId.getCustomID());
		...
		logDebugLocal("________________________________________________________________________________________");
		logDebugLocal("Total parcels added: " + totalParcelsAdded);
		logDebugLocal("\n");
		logDebugLocal("1. Failed to add a total of " + failedToAdd + " due to not able to meet the 19 digits requirement. Below is the list:");
		logDebugLocal(listOfFailedRecordIds);		
		logDebugLocal("\n");
		logDebugLocal("2. Failed to add a total of " + alreadyExistCnt + " since they already exist in parcel. Below is the list:");
		logDebugLocal(listofExistedRecordIds);
		logDebugLocal("________________________________________________________________________________________");
