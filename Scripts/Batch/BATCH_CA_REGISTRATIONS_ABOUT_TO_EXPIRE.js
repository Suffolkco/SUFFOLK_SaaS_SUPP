/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_CA_REGISTRATIONS_ABOUT_TO_EXPIRE.js 
| Trigger: Batch
| Client: Suffolk
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - BATCH_CA_REGISTRATIONS_ABOUT_TO_EXPIRE", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
                        ...
                        if (workflowResult.getSuccess())
                        {
                            ...
                            for (i in wfObj)
                            {
                                if (wfObj[i].getTaskDescription() == "Issuance")
                                {
                                    ...
                                    {
                                        aa.workflow.handleDisposition(capId, wfObj[i].getStepNumber(), wfObj[i].getProcessID(), "Pending Renewal", aa.date.getCurrentDate(), "Updated via BATCH_CA_REGISTRATIONS_ABOUT_TO_EXPIRE", "Updated via BATCH_CA_REGISTRATIONS_ABOUT_TO_EXPIRE", systemUserObj, "Y");
                                    }
                                }
                            }
                            aa.cap.updateAppStatus(capId, "Set to About to Expire from Batch", "About to Expire", sysDate, "Updated via BATCH_CA_REGISTRATIONS_ABOUT_TO_EXPIRE", systemUserObj);
                                ...
                                for (c in capContacts)
                                {
                                    if (capContacts[c].getCapContactModel().getContactType() == "Vendor")
                                    {
                                        ...
                                        if (!matches(capContacts[c].email, null, undefined, ""))
                                        {
                                            ...
                                            emailWithReportAttachASync(capContacts[c].email, "CA_REGISTRATION_ABOUT_TO_EXPIRE", vEParams, "CA Renewal Notifications SSRS V2", vRParams, "N", "");
                                        }
                                    }
                                }
                            }
