/*------------------------------------------------------------------------------------------------------/
| Program:EH_HIDE_INSPECTIONS_AFTER3YEARS.js: Batch
| 
| This batch script will run EOD to email Mike all the reports for violations found
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyehimslower@suffolkcountyny.gov", emailAddress, "", "Batch Job - EH_HIDE_INSPECTIONS_AFTER3YEARS", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
		...
		var startDate = new Date();		
		var todayDate = (startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear();	
		var dateAdd = addDays(todayDate, -1095);
		//var dateCheck = dateAdd(null, 0);
        dateCheckString = String(dateAdd).split("/")
        var dateToCheck = (String('0' + dateCheckString[0]).slice(-2) + '/' + String('0' + dateCheckString[1]).slice(-2) + '/' + dateCheckString[2]);
        var inspVisibleCnt = 0;
		var inspHiddenCnt = 0;
		var inspCnt = 0;

        logDebugLocal("Inspection Date check on : " + dateToCheck);	

		// SQL to see how many inspections are hidden or visible on ACA
		var vSQL = "Select DATE_INSPECTION as inspDate, INSPECTION_TYPE as inspType, INSPECTION_ID as inspectionId, RECORD_ID as recordNumber from V_INSPECTION where RECORD_MODULE = 'EnvHealth' and RECORD_TYPE in ('Food Service Permit', 'Limited/Mobile Permit')";
		//var vSQL = "Select DATE_INSPECTION as inspDate, INSPECTION_TYPE as inspType, INSPECTION_ID as inspectionId, RECORD_ID as recordNumber  from V_INSPECTION where INSPECTION_TYPE IN ('012 Premise/Facility Inspection', '12 Obs Premise/Facility Inspection') AND DATE_INSPECTION < '" + dateToCheck + "'";  		
        		
		var vResult = doSQLSelect_local(vSQL);  	     
		logDebugLocal("Pulling number of inspection records:" +  vResult.length);	
	
	
		for (r in vResult)
        {
            recordID = vResult[r]["recordNumber"];      
			inspID = vResult[r]["inspectionId"];    
			siteInspTYPE = vResult[r]["inspType"];             
            capId = getApplication(recordID);
            capIDString = capId.getCustomID();
            cap = aa.cap.getCap(capId).getOutput();
			var reportParams = aa.util.newHashtable();
			var reportTankParams = aa.util.newHashtable();
                ...
                if (capmodel.isCompleteCap())
                {
					...
					var inspResultObj = aa.inspection.getInspection(capId, inspID);				
						...
						{												
							...
							var displayInACA = actModel.getDisplayInACA();
							var restrictInACA = actModel.getIsRestrictView4ACA();
							
							if (displayInACA == 'Y')
							{
								logDebugLocal("Inspection is visible on ACA: " + capIDString + ", " + inspID);
								inspVisibleCnt++;
							}
							if (restrictInACA == 'Y')
							{
								logDebugLocal("Inspectin is hidden on ACA: " + capIDString + ", " + inspID);
								inspHiddenCnt++;
							}
							inspCnt++;
						}
					...
					/*
					inspDateCon = inspObj.getInspectionDate().getMonth() + "/" + inspObj.getInspectionDate().getDayOfMonth()+ "/" + inspObj.getInspectionDate().getYear();													
							logDebugLocal("Cap Id: " + capIDString + ",  Inspection Date: " + inspDateCon );
						if (matches(inspType.toString(), tankInspType))																											
						{	
							var inspDate =inspList[xx].getInspectionDate();
							inspObj = inspList[xx];
							inspDateCon = (inspDate.getMonth()) + "/" + inspDate.getDayOfMonth()+ "/" + inspDate.getYear();													
							
							var dateDiff = parseFloat(dateDifference(inspDate, dateToCheck));
							...
							if (dateDiff == 0)
							{
								...
								logDebugLocal("Tank ID is: " + childCapIdString.toString());	
								logDebugLocal("Tank Insp date is: " + inspDateCon);	
								...
								//logDebugLocal("Inspection DateTime: " + year + "-" + month + "-" + day + " " +  hr + ':' + min + ":" + sec + ".0");	
								var tankinspectionDateCon = year + "-" + month + "-" + day + " " +  hr + ':' + min + ":" + sec + ".0";
		...
		logDebugLocal("Total Inspections : " + inspCnt);    
		
		logDebugLocal("ACA visible : " + inspVisibleCnt); 

		logDebugLocal("ACA Restrict : " + inspHiddenCnt	); 
	}
    catch (err) 
    {
...
function getRecordParams4NotificationLocal(params) {
...
function generateReportBatch(itemCap, reportName, module, parameters)
{
    ...
    //logDebugLocal("user: " + user);
    //logDebugLocal("Report Name: " + reportName);
    var report = aa.reportManager.getReportInfoModelByName(reportName);
    if (!report.getSuccess() || report.getOutput() == null)
    {
        logDebugLocal("**WARN report generation failed, missing report or incorrect name: " + reportName);
        return false;
    }
    ...
    if (permit.getOutput().booleanValue())
    {
        ...
        {
            logDebugLocal("**WARN System failed get report: " + reportResult.getErrorType() + ":" + reportResult.getErrorMessage());
            return false;
        }
    } else
    {
        logDebugLocal("You have no permission.");
        return false;
    }
}
