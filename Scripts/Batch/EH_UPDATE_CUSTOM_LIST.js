/*------------------------------------------------------------------------------------------------------/
| Program:EH_UPDATE_CUSTOM_LIST.js: Batch
| 
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyehimslower@suffolkcountyny.gov", emailAddress, "", "Batch Job - EH_UPDATE_CUSTOM_LIST", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
    ...
    try 
    {
		...
		for (r in vResult)
        {
            ...
            if (cap)
            {
                ...
                {				
					deleteASITableRow(capId, 'PERMIT RESTRICTIONS') 			
...
/------------------------------------------------------------------------------------------------------*/
function deleteASITableRow(tableCapId, tableName) {
	logDebugLocal(tableCapId.getCustomID() + "|" + tableName);
	...
	if (tableArr) {		
		...
		for (var r in tableArr) {
			...
			for (var col in tableArr[r]) {
				logDebugLocal("In row " + r + ": " + tableArr[r][col].toString());
				if (tableArr[r][col].toString()  == 'CD - Cold Delivery Only' ||
				tableArr[r][col].toString()  == 'DD - Hot & Cold Delivery' ||
				tableArr[r][col].toString()  == 'HD - Hot Delivery Only' )
				{
					logDebugLocal("Do not add this row: " + tableArr[r][col].toString());					
					...
					addCnt++;
					logDebugLocal("Added row: "  + tableArr[r][col].toString());
					rowArr.push(tempArr); 			
		...
		logDebugLocal("Number of rows added: " + addCnt);
	}
}
