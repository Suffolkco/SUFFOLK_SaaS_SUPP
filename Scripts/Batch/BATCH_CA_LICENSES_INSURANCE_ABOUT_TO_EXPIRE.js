/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_CA_LICENSES_INSURANCE_ABOUT_TO_EXPIRE.js
| Trigger: Batch
| Client: Suffolk
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - BATCH_CA_LICENSES_INSURANCE_ABOUT_TO_EXPIRE", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
function mainProcess() 
{
    ...
    {       
        ...
        var vSQL = "SELECT B1.B1_ALT_ID as recordNumber, BC.B1_CHECKLIST_COMMENT as ExpDate FROM B1PERMIT B1 INNER JOIN BCHCKBOX BC on b1.serv_prov_code = bc.serv_prov_code and b1.b1_per_id1 = bc.b1_per_id1 and b1.b1_per_id2 = bc.b1_per_id2 and b1.b1_per_id3 = bc.b1_per_id3 and bc.B1_CHECKBOX_TYPE = 'INSURANCE INFORMATION' and bc.B1_CHECKBOX_DESC = 'Policy Expiration Date' and BC.B1_CHECKLIST_COMMENT IN ('" + dateToCheck + "','" + dateCheckString1 + "')  WHERE B1.SERV_PROV_CODE = 'SUFFOLKCO' and B1_PER_GROUP = 'ConsumerAffairs' and B1.B1_PER_TYPE = 'Licenses' and B1_PER_CATEGORY = 'NA'";
        //  
        var output = "Record ID | Insurance Policy Expiration Date \n";
                                    ...
                                    if (capContacts[c].getCapContactModel().getContactType() == "Vendor")
                                    {
                                        ...
                                        if (!matches(capContacts[c].email, null, undefined, ""))
                                        {
                                            if (appTypeArray[2] != "Polygraph Examiner") 
                                            {
                                                ...
                                                sendNotification("", conEmail, "", "CA_LICENSE_INSURANCE_ABOUT_TO_EXPIRE", vEParams, null);
                                            }                                           
                                        }
                                    }
                                }
