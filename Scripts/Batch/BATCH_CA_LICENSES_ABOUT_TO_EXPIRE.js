                        ...
                        if (workflowResult.getSuccess())
                        {
                            ...
                            for (i in wfObj)
                            {
                                if (wfObj[i].getTaskDescription() == "Issuance")
                                {
                                    if (wfObj[i].getDisposition() != "Pending Renewal")
                                    {
                                        aa.workflow.handleDisposition(capId, wfObj[i].getStepNumber(), wfObj[i].getProcessID(), "Pending Renewal", aa.date.getCurrentDate(), "Updated via BATCH_CA_LICENSES_ABOUT_TO_EXPIRE", "Updated via BATCH_CA_LICENSES_ABOUT_TO_EXPIRE", systemUserObj, "N");
                                    }
                                }
                            }
                            aa.cap.updateAppStatus(capId, "Set to About to Expire from Batch", "About to Expire", sysDate, "Updated via BATCH_CA_LICENSES_ABOUT_TO_EXPIRE", systemUserObj);
