/*------------------------------------------------------------------------------------------------------/
| Program: opcTightnessTestToExpire.js  Trigger: Batch
| Version 1.0 Bill Conner 03/22/2019
...
try 
{
    for (var i in rtArray) 
    {
        ...
        for (var j in recArray) 
        {
            ...
            if (cap)
            {
                var stMinus = new Date(dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), -90));
                //logDebug("Date Minus is: " + stMinus);
                ...
                if(capmodel.isCompleteCap())
                {
                    ...
                    if (b1ExpResult.getSuccess())
                    {
						...
						if (expDate != null)
						{
							var expDateCon = new Date(expDate.getMonth() + "/" + (expDate.getDayOfMonth() +1) + "/" + expDate.getYear());
							...
							if (expStatus == "About to Expire" && appStatus != "Inactive")
							{
								var dateDiff = parseFloat(dateDifference(expDateCon, startDate));
								...
								if (dateDiff >= 0)
								{
									...
									logDebug("dateDiff is: " + dateDiff);
									logDebug("Expiration status set to 'Expired'");
									...
									for (con in conArray)
									//{
									//	if (conArray[con].contactType == "Billing Contact" || conArray[con].contactType == "Tank Owner")
										{
											applicant = conArray[con];
											conEmail = applicant.email;
											if (conEmail != null)
											{
												sendNotification("", conEmail, "", "DEQ_OPC_TightnessTest_INACTIVE", params, reportFile);
											}
										}
									//}
								}
