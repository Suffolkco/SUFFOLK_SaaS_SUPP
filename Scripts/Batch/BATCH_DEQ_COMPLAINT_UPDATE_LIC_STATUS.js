/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_DEQ_COMPLAINT_UPDATE_LIC_STATUS.js
| Trigger: Batch
| Client: Suffolk
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        //aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - BATCH_DEQ_COMPLAINT_UPDATE_LIC_STATUS", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
function mainProcess() 
{
    try
    {
        var vSQL = "SELECT B1.B1_ALT_ID as recordNumber, BC.B1_CHECKLIST_COMMENT as licenseToCheck, B1_APPL_STATUS FROM B1PERMIT B1 INNER JOIN BCHCKBOX BC on b1.serv_prov_code = bc.serv_prov_code and b1.b1_per_id1 = bc.b1_per_id1 and b1.b1_per_id2 = bc.b1_per_id2 and b1.b1_per_id3 = bc.b1_per_id3 and bc.B1_CHECKBOX_TYPE = 'LICENSE INFORMATION' and bc.B1_CHECKBOX_DESC = 'License Number' and BC.B1_CHECKLIST_COMMENT <> '' WHERE B1.SERV_PROV_CODE = 'SUFFOLKCO' and B1_PER_GROUP = 'ConsumerAffairs' and B1.B1_PER_TYPE = 'Complaint' and B1_PER_CATEGORY = 'NA'";
                        ...
                        if (workflowResult.getSuccess())
                        {
                            ...
                            for (i in wfObj)
                            {
                                if (wfObj[i].getTaskDescription() == "Issuance")
                                {
                                    ...
                                    {
                                        aa.workflow.handleDisposition(capId, wfObj[i].getStepNumber(), wfObj[i].getProcessID(), "Pending Renewal", aa.date.getCurrentDate(), "", "", systemUserObj, "Y");
