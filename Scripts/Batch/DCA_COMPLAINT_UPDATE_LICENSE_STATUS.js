/*------------------------------------------------------------------------------------------------------/
| Program:DCA_COMPLAINT_UPDATE_LICENSE_STATUS.js: Batch
| 
| This batch script will run ONE TIME to update Shevled status to EXPIRED if it has expired.
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - DCA_COMPLAINT_UPDATE_LICENSE_STATUS", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
    ...
    try 
    {
		...
		for (r in vResult)
        {
            ...
            if (cap)
            {
                ...
                {					
					compLicenseStatus = getAppSpecific("License Status");	
					compLicenseNo = getAppSpecific("License Number");	
					
					if (compLicenseNo != null && compLicenseNo != null)
					{
						logDebugLocal(capIDString + " has custom field license status of " + compLicenseStatus);
						logDebugLocal("Custom Field License number on Complaint is " + compLicenseNo);

						licenseCapId = getApplication(compLicenseNo);
						
						if (licenseCapId) 
						{
							appStatus = getAppStatus(licenseCapId).toUpperCase();
							logDebugLocal("appStatus is: " + appStatus);
							...
							if (licList && licList != "")
							{
								...
								logDebugLocal("Updated " + capIDString + "  custom field icense Status from " + compLicenseStatus + " to " + licStatus);						
								count++;
							}
							else
							{
								...
								switch (licStatus) {
									...
									case "Pending Renewal":
										newlicStatus = 'Licensed';
										countLic++;	
										break;
									case "License Suspended":
										newlicStatus = 'Suspended';
										countSusp++;
										break;
									default:
										newlicStatus = "Unlicensed";	
										countUnLic++;									
									break;
								}
								editAppSpecific("License Status", newlicStatus);
								logDebugLocal("Updated " + capIDString + "  custom field License Status from " + compLicenseStatus + " to " + newlicStatus);							
							
							}
						}
						else
						{
							logDebugLocal(capIDString + " has invalid custom field license number listed: " + compLicenseNo);
							countInvalid++;
						}
		...
		logDebugLocal("Total of record that has been updated with matching License Status: " + count + ".");
		logDebugLocal("Total of record updated to Licensed: " + countLic + ".");
		logDebugLocal("Total of record updated to Unlicensed: " + countUnLic + ".");
		logDebugLocal("Total of record updated to Suspended: " + countSusp + ".");	
		logDebugLocal("Total of record that has invalid License Number: " + countInvalid + ".");
