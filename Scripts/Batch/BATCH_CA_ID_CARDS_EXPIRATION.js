...
function mainProcess() 
{
    try 
    {
        ...
        var vSQL = "SELECT B1.B1_ALT_ID as recordNumber, BC.B1_CHECKLIST_COMMENT as ExpDate FROM B1PERMIT B1 INNER JOIN BCHCKBOX BC on b1.serv_prov_code = bc.serv_prov_code and b1.b1_per_id1 = bc.b1_per_id1 and b1.b1_per_id2 = bc.b1_per_id2 and b1.b1_per_id3 = bc.b1_per_id3 and bc.B1_CHECKBOX_TYPE = 'LICENSE DATES-SALES' and bc.B1_CHECKBOX_DESC = 'Expiration Date' and BC.B1_CHECKLIST_COMMENT = '" + dateCheckString + "'   WHERE B1.SERV_PROV_CODE = 'SUFFOLKCO' and B1_PER_GROUP = 'ConsumerAffairs' and B1.B1_PER_TYPE = 'ID Cards' and B1_PER_CATEGORY = 'NA' ";
                        ...
                        if (workflowResult.getSuccess())
                        {
                            ...
                            for (i in wfObj)
                            {
                                if (wfObj[i].getTaskDescription() == "Issuance")
                                {
                                    ...
                                    {
                                        aa.workflow.handleDisposition(capId, wfObj[i].getStepNumber(), wfObj[i].getProcessID(), "Expired", aa.date.getCurrentDate(), "Updated via BATCH_CA_ID_CARDS_EXPIRATION", "Updated via BATCH_CA_ID_CARDS_EXPIRATION", systemUserObj, "Y");
                                    }
                                }
                            }
                            aa.cap.updateAppStatus(capId, "Set to Expired from Batch", "Expired", sysDate, "Updated via BATCH_CA_ID_CARDS_EXPIRATION", systemUserObj);
