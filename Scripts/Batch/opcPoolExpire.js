/*------------------------------------------------------------------------------------------------------/
| Program: opcTankExpire.js  Trigger: Batch
| 
| This batch script will run daily to send email notification to Pool owner and operator 
| on the day of the the expiration 
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
if (paramsOK) 
{
	...
	if (!timeExpired) 
	{
		...
		aa.sendMail("noreplyehims@suffolkcountyny.gov", emailAddress, "", "Batch Job - OPC Pool Expiration", emailText);
	}
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
				...
				if (cap)
				{
					...
					if(capmodel.isCompleteCap())
					{
						...
						if (b1ExpResult.getSuccess())
						{
							...
							if (curExp != null)
							{
								//var expDateCon = new Date(curExp.getMonth() + "/" + (curExp.getDayOfMonth() +1) + "/" + curExp.getYear());
								
								var expStatus = b1Exp.getExpStatus();

								if (expStatus == "About to Expire" && appStatus == "Active")
								{									
									
									logDebug("Record: " + capIDString );
									

									var expDateCon = curExp.getMonth() + "/" + curExp.getDayOfMonth() + "/" + curExp.getYear();
									logDebug("Expiration Date is: " + expDateCon);					
								...
								// Due today	
									if (dateDiff == 0)
									{
										...
										updateAppStatus("Expired", "Updated via opcPoolExpire script", capId);
										...
										for (con in conArray)
										{
											if (conArray[con].contactType == "Pool Owner" || conArray[con].contactType == "Operator")
											{
												...
												sendNotification("", conEmail2, "", "DEQ_OPC_POOL_EXPIRE_TODAY", emailParams, reportFile);
