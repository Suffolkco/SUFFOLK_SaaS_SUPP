/*------------------------------------------------------------------------------------------------------/
| Program: DCA_PET_YEARLY_INSEPCTION.js  Trigger: Batch| 
| 
| This batch script will run daily to send email notification to four users yearly on 8/18
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
aa.env.setValue("userFirstNameList","Danielle, Claudette");
aa.env.setValue("userEmailList","Danielle.Grucci-Petersen@suffolkcountyny.gov, Claudette.Lea@suffolkcountyny.gov")

var userFirstNameArray = getParam("userFirstNameList").split(",");
var userEmailAddressArray = getParam("userEmailList").split(",");
...
var rtArray = ["ConsumerAffairs/Inspections/Pet Stores/NA"];
var emailSender = "monthlycalicensingrenewals@suffolkcountyny.gov";	
/*----------------------------------------------------------------------------------------------------/
|
...
if (paramsOK) 
{
	...
	if (!timeExpired) 
	{
		...
		aa.sendMail(emailSender, emailAddress, "", "Batch Job - DCA_PET_YEARLY_INSPECTION", emailText);
	}
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
function mainProcess() 
{
    ...
    {       
        var conEmail = [];             
        var	emailContent;
        var emailSubject = "All annual pet inspection records are due for the annual inspection review";


        for (xx in userFirstNameArray) 
        {            
            emailContent = "Hello " + userFirstNameArray[xx] + ",<br><br>"
                            + "Attention, all your annual pet inspection records are due for the annual inspection review. <br><br>"                         
                            + "Thank you! <br>"
                            + "Note: This message is automatcally generated by Accela.";

            conEmail = userEmailAddressArray[xx];		

            aa.sendEmail(emailSender, conEmail, "", emailSubject , emailContent , "");		             
