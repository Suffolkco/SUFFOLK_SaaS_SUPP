/*------------------------------------------------------------------------------------------------------/
| Program:DEQ_OPC_UPDATE_CUSTOM_FIELD.js: Batch
| 
...
{    
    try 
    {
        ...
        var vTankSQLResult = doSQLSelect_local(vTankSQL);
       	var statusVal = "99-Other";
		var otherStatusVal = "Administratively Closed";
		var NIStatusUpdate = 0;
		var NIOtherStatusUpdate = 0;
		var REMURStatusUpdate = 0;
		var REMUROtherStatusUpdate = 0;
		var URStatusUpdate = 0;		
		var UROtherStatusUpdate = 0;
					...
					if (childArray)
					{
						...
						{							
							...
							{							
								...
								if (offUseCode != null)
								{			
									var status = getAppSpecific("Status", childCapId);   						
									var otherStatus = getAppSpecific("Other Status", childCapId);  

									if (offUseCode == 'NI')
									{	
										if (status != statusVal)
										{
											//editAppSpecific("Status", statusVal, childCapId);
											NIStatusUpdate++;
										}
										if (otherStatus != otherStatusVal)
										{
											//editAppSpecific("Other Status", otherStatusVal, childCapId);
											NIOtherStatusUpdate++;
										}
									}
									else if (offUseCode == "REMUR")
									{
										if (status != statusVal)
										{
											//editAppSpecific("Status", statusVal, childCapId);
										...
										if (otherStatusVal != "Unregistered Closed-Removed")
										{
											//editAppSpecific("Other Status", "Unregistered Closed-Removed", childCapId);
											REMUROtherStatusUpdate++;
										}
									}
									else if (offUseCode == "UR")
									{
										if (status != statusVal)
										{
											//editAppSpecific("Status", statusVal, childCapId):
										...
										if (otherStatusVal != "Unregistered")
										{
											//editAppSpecific("Other Status", "Unregistered", childCapId);
											UROtherStatusUpdate++;
										}
									}
		...
		logDebugLocal("Batch # 1: Number of NI Tank records that needs to update Status to 99-Other: " + NIStatusUpdate);
		logDebugLocal("Batch # 1: Number of NI Tank records that needs to update Status to Administratively Closed: " + NIOtherStatusUpdate);
		logDebugLocal("Batch # 1: Number of REMUR Tank records that needs to update Status to 99-Other: " + REMURStatusUpdate);
