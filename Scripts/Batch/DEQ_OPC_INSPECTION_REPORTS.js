/*------------------------------------------------------------------------------------------------------/
| Program:DEQ_OPC_INSPECTION_REPORTS.js: Batch
| 
...
{    
    try 
    {
		...
		for (r in vResult)
        {
            ...
            if (cap)
            {
                ...
                if (capmodel.isCompleteCap())
                {
					...
					var status = getAppStatus(capId);
					//logDebugLocal("Status: " + status);
						...
						{							
							...
							if (siteDateDiff == 0)
							{
								...
								if (inspObj.getInspectionStatus() == "Violations Found")
								{
									...
									//logDebugLocal("inspectionDateCon: " + inspectionDateCon);

									addParameter(reportParams, "SiteRecordID", capIDString);					
								...
								// Site inspection type
								//logDebugLocal("Site Insepction type: " + inspObj.getInspectionType());
								var tankInspType = String(inspObj.getInspectionType()).replace(/Site/g,'Tank');
								tankInspType = String(tankInspType.replace(/OPC /g,''));
															
								// Find child of the site							
											...
											if (childCapStatus == "Active")
											{
												...
												if (inspResultObj.getSuccess())
												{
													...
													for (xx in inspList)
													{
														...
														childCapIdString = childCapId.getCustomID();

														if (inspList[xx].getInspectionStatus() == "Violations Found")
														{
															//logDebugLocal(tankInspType + " =? " + inspType);
														
															// Only if it matches with the SITE inspection type, we proceed.													

															// (inspType == "Non-PBS Tank OP Inspection" ||
															// == "Non-PBS Tank Other Inspection" ||
															// == "Non-PBS Tank Re-Inspection" ||
														...
														{
															//logDebugLocal("Active tank: " + childCapIdString + " do not have matching site inspection: " + inspType);
														}
													}
												}
											}
		...
		logDebugLocal("Number of sites with inspection that sent emails: " + siteCount);      
		logDebugLocal("Number of tanks with inspection that sent emails: " + tankCount);      
	}
    catch (err) 
    {
        logDebugLocal("**ERROR** runtime error " + err.message + " at " + err.lineNumber + " stack: " + err.stack);
