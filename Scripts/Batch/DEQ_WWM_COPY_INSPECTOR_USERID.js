/*------------------------------------------------------------------------------------------------------/
| Program:DEQ_WWM_COPY_INSPECTOR_USERID.js: Batch
| 
| This batch script will run often to copy the inspector ID of today's WWM inspection 
| to record Inspector UserID 
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
{    
    try 
    {
        ...
        //var vSQL = "SELECT B1.B1_ALT_ID as recordNumber FROM B1PERMIT B1 WHERE B1.B1_ALT_ID = 'R-22-0013'";

		var vSQL = "SELECT B.B1_ALT_ID as recordNumber FROM B1PERMIT B JOIN BPERMIT_DETAIL C ON B.B1_PER_ID1 = C.B1_PER_ID1 AND B.B1_PER_ID2 = C.B1_PER_ID2 AND B.B1_PER_ID3 = C.B1_PER_ID3 WHERE B.REC_STATUS = 'A' AND B.B1_PER_TYPE = 'WWM' AND B.B1_PER_SUB_TYPE = 'Residence' AND B.B1_PER_CATEGORY = 'Application' AND C.B1_SCHEDULED_DATE = '" + dateToCheck + "'";
		
        var output = "Record ID\n";  		
        
		// SQL to pull active OPC site records that has NO child Tank records		
		var vResult = doSQLSelect_local(vSQL);  	     
		logDebugLocal("Pulling number of records:" +  vResult.length);	
		...
		for (r in vResult)
        {
            ...
            if (cap)
            {
                ...
                if (capmodel.isCompleteCap())
                {
					...
					//logDebugLocal("Getting Existing record scheduled date: " + cd.getScheduledDate().getMonth() + "/" + cd.getScheduledDate().getDayOfMonth() + "/" + cd.getScheduledDate().getYear());					
					logDebugLocal("Setting schedued date for record: " + capIDString);			
					capId = getApplication(recordID);
					...
					{						
						...
						logDebugLocal("Inspection object lenght: " + inspList.length);
						for (xx in inspList)
						{
							logDebugLocal("type: " + inspList[xx].getInspectionType() + " status: " + inspList[xx].getInspectionStatus().toUpperCase());

							//logDebugLocal("Inspection Date: " + inspList[xx].getScheduledDate());
							//logDebugLocal("dateToCheck Date: " + dateToCheck);
							inspectorFirstName = inspList[xx].getInspector().getFirstName();
							inspectorMiddleName = inspList[xx].getInspector().getMiddleName();
							inspectorLastName = inspList[xx].getInspector().getLastName();

							if (inspList[xx].getInspectionStatus().toUpperCase() == 'SCHEDULED')
							{								
								//logDebugLocal("Inspection ID number detected : " +  inspList[xx].getIdNumber());
								// Copy inpsector name to record field
								...
								if (inspUserObj.getSuccess())
								{
									...
									if (!cdWrite.getSuccess()){ 
										...
										logDebugLocal("Updated insepctor ID  to: " + inspectorObj.getUserID());
									 }
								}	


								//updateRecordcheduledDateLocal("Test", capId);
...
/------------------------------------------------------------------------------------------------------*/
function updateRecordInspectorLocal(firstName, MiddleName, LastName){ // option CapId
	var itemCap = capId
	...
	try
	{
		...
		inspUserObj = aa.person.getUser(firstName, MiddleName, LastName);     
		logDebugLocal("inspUserObj: " + inspUserObj);
		if (inspUserObj.getSuccess())
		{
			logDebugLocal("Set inspector");
