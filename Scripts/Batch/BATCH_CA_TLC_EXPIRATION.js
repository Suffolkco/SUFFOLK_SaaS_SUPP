...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("monthlycalicensingrenewals@suffolkcountyny.gov", emailAddress, "", "Batch Job - BATCH_CA_TLC_EXPIRATION", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
                        ...
                        if (workflowResult.getSuccess())
                        {
                            ...
                            for (i in wfObj)
                            {
                                if (wfObj[i].getTaskDescription() == "Issuance")
                                {
                                    ...
                                    {
                                        aa.workflow.handleDisposition(capId, wfObj[i].getStepNumber(), wfObj[i].getProcessID(), "Expired", aa.date.getCurrentDate(), "Updated via BATCH_CA_TLC_EXPIRATION", "Updated via BATCH_CA_TLC_EXPIRATION", systemUserObj, "Y");
                                    }
                                }
                            }
