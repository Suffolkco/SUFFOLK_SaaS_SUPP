/*------------------------------------------------------------------------------------------------------/
| Program: iaAppNextServiceNotify.js  Trigger: Batch
| Version 1.0 Ryan Littlefield 1/2/2018
...
var recType = ["DEQ/Ecology/IA/Application"];
/*----------------------------------------------------------------------------------------------------/
|
...
try 
{
    for (var i in recType) 
    {
        ...
        for (var j in recArray) 
        {
            ...
            if (cap)
            {
                logDebug("<b>" + capIDString + "</b>"); //comment this out
				var datePlusThirty = new Date(dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), 30));
				var datePlusSixty = new Date(dateAdd((startDate.getMonth() + 1) + "/" + startDate.getDate() + "/" + startDate.getFullYear(), 60));
                var dateCheck = false;
				var nextServiceDate = new Date(getAppSpecific("Next Service Date"));
				var nextServiceDateConverted = ((nextServiceDate.getMonth() + 1) + "/" + nextServiceDate.getDate() + "/" + nextServiceDate.getFullYear());
                var datePlusThirtyConverted = ((datePlusThirty.getMonth() + 1) + "/" + datePlusThirty.getDate() + "/" + datePlusThirty.getFullYear());
				var datePlusSixtyConverted = ((datePlusSixty.getMonth() + 1) + "/" + datePlusSixty.getDate() + "/" + datePlusSixty.getFullYear());
				var dateCheckSixty;

				logDebug(nextServiceDateConverted + " is the service date we're checking against");
				logDebug(datePlusThirtyConverted + " this is 30 days in the future");
                logDebug(datePlusSixtyConverted + " this is 60 days in the future");
                if (datePlusThirtyConverted == nextServiceDateConverted)
                {
                    dateCheck = true;
                }
                if ((dateCheck == true)) 
                {
					...
					if (capContactResult.getSuccess()) 
					{
						...
						for (con in contacts)
						{
							var newContact = contacts[con].getCapContactModel();
							//debugObject(newContact);
							if(newContact.getPeople().getContactType() == "Property Owner")
							{
								if (newContact.getPeople().getAuditStatus() == "A") 
								{ 
									conEmail += newContact.email + ", ";
								}
							}
						}
					}
                    ...
                    if (conEmail != null)
                    {
                    sendNotification("", lpEmail, conEmail, "IA_NEXT_SERVICE_DUE_DATE", params, reportFile);
                    }
				}
				if (datePlusSixtyConverted == nextServiceDateConverted)
                {
                    dateCheckSixty = true;
                }
                if ((dateCheckSixty == true)) 
                {
                    //logDebug("<b>" + capIDString + "</b>");
                    var params = aa.util.newHashtable();
                    //var conArray = getContactArray();
                    ...
                    if (conEmail != null)
                    {
                    sendNotification("", lpEmail, conEmail, "IA_NEXT_SERVICE_DUE_DATE_60", params, reportFile);
                    }
                }
            }
        }
