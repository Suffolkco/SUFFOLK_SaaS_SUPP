/*------------------------------------------------------------------------------------------------------/
| Program: tightnessTestAboutToExpire30-60.js  Trigger: Batch
| Client: JCCAL
| Version 1.0 Casey Gray 3/20/2019
| This batch script will daily.
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
var rtArray = ["DEQ/OPC/Tightness Tester/NA", "DEQ/OPC/Tightness Test Company/NA"];
/*----------------------------------------------------------------------------------------------------/
|
...
if (paramsOK) 
{
	...
	if (!timeExpired) 
	{
		...
		aa.sendMail("noreplyehims@suffolkcountyny.gov", emailAddress, "", "Batch Job - tightnessTestAboutToExpire3060", emailText);
	}
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
                        ...
                        if (b1ExpResult.getSuccess())
                        {
                            ...
                            if (curExp != null)
                            {
                                ...
                                if (curSt != null)
                                {
									...
									{
										feeObjArr = assessedFees.getOutput();
										for (ff in feeObjArr)
										{
											fee = feeObjArr[ff];
											if(!matches(fee.getFeeitemStatus(), "VOIDED", "CREDITED", "NEW"))
											{
												totalFeeAmount += fee.getFee();	
											}
										}													
										
									}
									logDebug("Total Fee Amount: " + fee);
									// Get Applicant Info
									var conArray = getContactArray();
										for (con in conArray)
										{
											if (conArray[con].contactType == "Applicant")
											{
												...
												orgName = applicant.businessName;
												address1 = applicant.addressLine1;				
												city = applicant.city;
                                    ...
                                    if (dateDifRound == 60)
                                    {
										...
										addParameter(emailParams, "$$expireDate$$", curExpCon);
										addParameter(emailParams, "$$applicant$$", appName);
										addParameter(emailParams, "$$orgName$$", orgName);
										...
										logDebug("In email param: Fee Amount: " + totalFeeAmount.toFixed(2));	
										addParameter(emailParams, "$$amount$$", totalFeeAmount.toFixed(2));
                                        ...
                                        for (var lp in lpArr)
                                        {
											if (appTypeArray[2] == "Tightness Tester")
											{		
												if (lpArr[lp].getLicenseType() != "Tightness Test Company")
												{
													if (!matches(lpArr[lp].getEmail(), null, undefined, ""))
													{
														conEmail.push(lpArr[lp].getEmail());
													}
												}
											}
											else if (appTypeArray[2] == "Tightness Test Company")
											{
												if (lpArr[lp].getLicenseType() == "Tightness Test Company")
												{
													if (!matches(lpArr[lp].getEmail(), null, undefined, ""))
                                        ...
                                        if (conEmail != null)
                                        {
                                            for (c in conEmail)
                                            {
												if (thisType == "DEQ/OPC/Tightness Tester/NA")
												{
													logDebug("In record type sending email: " + thisType);
													sendNotification("", conEmail[c], "", "DEQ_OPC_TIGHTNESS TESTER_ABOUT_TO_EXPIRE_30_60", emailParams, null);
												}
												else
												{
													logDebug("In record type sending email: " + thisType);
													sendNotification("", conEmail[c], "", "DEQ_OPC_TIGHTNESS TESTING_COMPANY_ABOUT_TO_EXPIRE_30_60", emailParams, null);
												}
                                            }
                                        }
                                    }
