...
{    
    try {
        ...
        var publicUserID = "PUBLICUSER17533";        // MUST CHANGE: This needs to be the public user ID
        var licenseNum = "H-39881";                 // MUST CHANGE: This needs to be the license #
        ...
        logDebugLocal("Find public user: " + publicUserID);
           if (getUserResult.getSuccess() && getUserResult.getOutput()) {
            ...
            if (refContact != null) {
                ...
                logDebugLocal("Found reference contact for public user: " + refContactNum)
                // ADD CONTACT TO ~THIS~ PIN RECORD
                var capResult = aa.cap.getCapID(licenseNum);
                if (capResult.getSuccess()) {
                    recCapId = capResult.getOutput();
                addRefContactToRecordFunc(refContactNum, "Public User", recCapId);
                logDebugLocal("Added reference contact for public user: " + refContactNum + " to " + licenseNum);  
                }              
            }
        }        
    }
    catch (err) {
        aa.print("A JavaScript Error occurred in BATCH_CA_CONNECT_PUBLIC_USER_PIN: " + err.message);
    }    
...
function addRefContactToRecordFunc(refNum, cType, recCapId) {
    itemCap = recCapId;
