/*------------------------------------------------------------------------------------------------------/
| Program:DEQ_WWM_UPDATE_REFERENCE_CONTACT.js Batch
| 
...
if (paramsOK) 
{
    ...
    if (!timeExpired) 
    {
        ...
        aa.sendMail("noreplyehimslower@suffolkcountyny.gov", emailAddress, "", "Batch Job - DEQ_WWM_UPDATE_REFERENCE_CONTACT", emailText);
    }
}
/*------------------------------------------------------------------------------------------------------/
| <===========End Main=Loop================>
...
{    
    try 
    {
		...
		// TEST data
		var vSQL = "select distinct CONTACT_REF_ID as contactId from V_CONTACT where RECORD_MODULE = 'DEQ' and EMAIL like 'ada.chan@suffolkcountyny.gov' and CONTACT_REF_ID is NOT NULL";
		var newEmail = 'ada.ariosto@gmail.com';
        //var vSQL = "select distinct CONTACT_REF_ID as contactId from V_CONTACT where RECORD_MODULE = 'DEQ' and EMAIL like 'BGrogan@PWGrosser.com' and CONTACT_REF_ID is NOT NULL";
		//var newEmail = 'SCWWM@PWGrosser.com';
		var count = 0;
			...
			if (refConResult.getSuccess()) {
				...
				if (refPeopleModel != null) {
					...
					if (result.getSuccess()) {
						logDebugLocal("Successfully update the contact " + refNum + " with email address");
						count++;
					}	
					else{
