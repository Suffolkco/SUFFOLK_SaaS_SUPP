/*------------------------------------------------------------------------------------------------------/
| Program : ACA_IA_SERVICE_DOC_ATTACHED.js
| Event   : ACA_Before
|
...
try
{
    var serviceType = AInfo["Service Type"];
    var conUp = AInfo["Contract Update"];
    var serRep = AInfo["Service Report"];
    ...
    if(serviceType == "Start Up" || conUp == "CHECKED")
    {
        ...
        if(!conCheck)
        {
            if (serviceType == "Start Up")
            {
                cancel = true;
                showMessage = true;
                comment("If 'Start Up' is selected as 'Service Type', a contract is required with this application.");
            }
            if (conUp == "CHECKED")
            {
                cancel = true;
                showMessage = true;
                comment("If 'Contract Update' is checked, a contract is required with this application.");
            }
        }
    }
    if(serRep == "CHECKED")
    {
        var servCheck = determineACADocumentAttached("Service Report");
        if (!servCheck) 
        {
            cancel = true;
            showMessage = true;
            comment("If 'Service Report' is checked, a Service Report is required with this application.");
        }
    }
    if(samRes == "CHECKED")
    {
        var samCheck = determineACADocumentAttached("Sample Results");
        if (!samCheck) 
        {
            cancel = true;
            showMessage = true;
            comment("If 'Sample Results' is checked, Sample Results are required with this application.");
        }
    }
