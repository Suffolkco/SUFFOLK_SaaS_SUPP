...
|
| Usage   : Developed by Zachary McVicker. 01/18/2019
|
...
try {
   ...
   copyOwner(parentCapId, capId);
	...
	var inspArray = [];
	aa.print("Has Inspections: " + inspections.getSuccess());
	if (inspections.getSuccess()) 
	{
		insps = inspections.getOutput();
		for (i in insps) 
		{
			aa.print("Inspection Type: " + insps[i].getInspectionType());
			if ((insps[i].getInspectionType() == "I/A") && (insps[i].getInspectionStatus() == "Complete" || insps[i].getInspectionStatus() == "Satisfactory"))
			{
				aa.print("I/A exists and is complete");
				inspArray.push(insps[i]);
			}
		}
		if (inspArray.length >= 1)
		{
		var latestIA = inspArray.slice(-1)[0];
		var inspDate = latestIA.getInspectionDate()
			if (inspDate != undefined)
			{
			var inspDateCon = inspDate.getMonth() + "/" + inspDate.getDayOfMonth() + "/" + inspDate.getYear();
			aa.print("Latest I/A inspection date is " + inspDateCon);
			}
		}
  	}

  	var inspectionID = AInfo["Inspection Number"];
	...
	editAppSpecific4ACA("Model Number", modelNumber, capId);
	editAppSpecific4ACA("Installation Date", inspDateCon, capId);
	editAppSpecific4ACA("WWM Application Number", wwmApp, capId);
	editAppSpecific4ACA("Inspection Number", inspApp, capId);
