/*------------------------------------------------------------------------------------------------------/
| Program : ACA IA APP ONLOAD.js
| Event   : ACA OnLoad Event
|
| Usage   : Developed by Zachary McVicker & Case Gray. 01/18/2019 
|
| Client  : Suffolk County
|//
|
| Notes   Reequires the WWM Record Number & the Insp ID of an inspection.
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
/------------------------------------------------------------------------------------------------------*/                                 //var showDebug = false;                                                 // Set to true to see results in popup window
var showMessage = false;                                              // Set to true to see debug messages in popup window
...
try {
  ...
  var inspectionVal = false;
  var wwmNum = aa.cap.getCapID(AInfo["WWM Application Number"]);
  var wwmNumStr = AInfo["WWM Application Number"];
  var insNum = AInfo["Inspection Number"];
  var iaArray = ["DEQ/Ecology/IA/Application"];
  
  //Validating WMM Number
  if(wwmNum.getSuccess())
  {
      //Checks for valid WWM Number
      var capId = wwmNum.getOutput();
      var inspections = aa.inspection.getInspections(capId);
  
        if (isLicenseType("DEQ/WWM/Commercial/Application",capId))
        {
            isCommercial = true;
        }
        if (isLicenseType("DEQ/WWM/Residence/Application",capId))
        {
            isResidential = true;
        }
        if(!isCommercial && !isResidential)
        {
            cancel=true;
            showMessage=true;
            comment("Please enter a valid WWM Application Number, line 124");
        }
      
      if (inspections.getSuccess()) 
      {
          //Checks for valid Inspection Number
            insps = inspections.getOutput();
            for (i in insps) 
            {
                if ((insps[i].getInspectionType() == "I/A") && (insps[i].getInspectionStatus() == "Complete" || insps[i].getInspectionStatus() == "Satisfactory"))
                {
					var inspSeq = insps[i].getIdNumber();
                    if (inspSeq.toString() == insNum.toString())
                    {
                        inspectionVal = true;
                    }
                }
            ...
            if(!inspectionVal)
            {
                cancel=true;
                showMessage=true;
                comment("Please enter a valid I/A Inspection Number");
            } 
      }
      else
      {
      ...
      for (var i in iaArray) 
      {
          var thisType = iaArray[i];
          ...
          {
              wwmRec = aa.cap.getCapID(recArray[j].getID1(), recArray[j].getID2(), recArray[j].getID3()).getOutput();
              capIDString = wwmRec.getCustomID();
              cap = aa.cap.getCap(wwmRec).getOutput();	
              if (cap)
              {
                  var appSpecInfoResult = aa.appSpecificInfo.getByCapID(wwmRec);
                  if (appSpecInfoResult.getSuccess()) 
                  {
                      var appspecObj = appSpecInfoResult.getOutput();
                      for (i in appspecObj) 
                      {
                          if (appspecObj[i].getCheckboxDesc() == "WWM Application Number")
                          {
                              var wwmNumber = appspecObj[i].getChecklistComment();
                          }
                          if (appspecObj[i].getCheckboxDesc() == "Inspection Number")
                          {
                              var inspectionNumber = appspecObj[i].getChecklistComment();
                          }
                          if (wwmNum != null)
                          {
                              if (wwmNumber == wwmNumStr)
                              {
								//Agency wants to be able to create a new IA application when using a previously used WWM Application Number
								/*
								cancel=true;
								showMessage=true;
								comment("This WWM Application Number has already been used");
								break outerLoop;
								*/
                              }
                          }
                          if (insNum != null)
                          {
                              if (inspectionNumber == insNum)
                              {
                                  cancel=true;
                                  showMessage=true;
                                  comment("This Inspection Number has already been used");
								  break outerLoop;
                              }
                          }
                      } 
                  ...
                  { 
                      //comment("**ERROR: getting app specific info for Cap : " + appSpecInfoResult.getErrorMessage());
                  }       
              }
          }
      }
  ...
  else
  {
        cancel=true;
        showMessage=true;
        comment("Please enter a valid WWM Application Number");  
  } 
}	

catch (err) { comment("**ERROR : " + err); }
...
{
var matchArray; //default to current app
...
function editAppSpecific4ACA(itemName, itemValue, cap) {
  ...
  field.getCheckboxDesc())) || itemName.equals(field.getCheckboxDesc())) {
                    ...
                    field.setChecklistlogDebug(itemValue);
