/*------------------------------------------------------------------------------------------------------/
| Program : ACA TANK APPLICATION BEFORE.js
| Event   : ACA Before Event
|
| Usage   : Developed by Casey Gray. 04/05/2019 
|
...
loadASITables4ACATANK();
    ...
    if(typeof(TANKINFORMATION)=="object")
    {
        /*var scopeCheck = false;
        for (t in TANKINFORMATION)
        {
            var scope = TANKINFORMATION[t]["Scope of Tank Work"];
            //logDebug("Scope is: " + scope);
            if (scope == "1-New Installation" || scope == "2-Registration")
            {
                scopeCheck = true;
                break;
            }
        ...
        if (scopeCheck == false)
        {
            cancel = true;
            showMessage = true;
            comment("Save Application and contact OPC with the Temp Record Number (e.g. ##TMP-######).")
        }*/
    }
    else
    {
      cancel = true;
      showMessage = true;
      comment("Tank Information Missing. Please add at least one tank.")
    }
}
