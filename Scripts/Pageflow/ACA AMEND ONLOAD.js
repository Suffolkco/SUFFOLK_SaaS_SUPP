...
function copy()
{
    ...
    logDebugLocal("TargetCapId:" + targetCapId);
    if(targetCapId==null)
    {
    ...
    {  
        ...
        logDebugLocal("Copy LP from:" + parentCapId + " to " + targetCapId);
        copyLicenseProfessional(parentCapId, targetCapId);
        logDebugLocal("Copy Address from:" + parentCapId + " to " + targetCapId);
        copyAddress(parentCapId, targetCapId);
        logDebugLocal("Copy Parcel from:" + parentCapId + " to " + targetCapId);
        copyParcel(parentCapId, targetCapId);
      
        //copyPeople(parentCapId, targetCapId);
        logDebugLocal("Copy owner from:" + parentCapId + " to " + targetCapId);
        copyOwner(parentCapId, targetCapId);
      
        //copyContacts(parentCapId, targetCapId);
        logDebugLocal("Copy condition from:" + parentCapId + " to " + targetCapId);
	      copyCapCondition(parentCapId, targetCapId);
        logDebugLocal("Copy additional info from:" + parentCapId + " to " + targetCapId);
        copyAdditionalInfo(parentCapId, targetCapId);
        logDebugLocal("Copy cap detail from:" + parentCapId + " to " + targetCapId);
        copyCapDetailInfo(parentCapId, targetCapId);
        logDebugLocal("Copy work des from:" + parentCapId + " to " + targetCapId);
        copyCapWorkDesInfo(parentCapId, targetCapId);
        logDebugLocal("Copy ASI table from:" + parentCapId + " to " + targetCapId);
        ...
        var cap = aa.cap.getCap(targetCapId).getOutput();	
            ...
            logDebugLocal("appTypeString:" + appTypeString);
            if(appTypeArray[0] == "DEQ" && appTypeArray[1] == "OPC" && appTypeArray[2] == "Swimming Pool" && appTypeArray[3] == "Amendment")
            {
                ...
                editContactType("Applicant","Pool Owner", targetCapId);
                var amendCapModel = aa.cap.getCapViewBySingle4ACA(targetCapId);
                useAppSpecificGroupName = false;
                var watSupply = getAppSpecific("Water Supply", parentCapId);
                var sewSystem = getAppSpecific("Sewerage System", parentCapId);
                var chUse = getAppSpecific("Change in Use", parentCapId);
                var proSco = getAppSpecific("Scope of Project", parentCapId);
                var salt = getAppSpecific("Saltwater-Chlorine Generator Installation *", parentCapId);
                var chemFeed = getAppSpecific("Upgrading Chemical Feeders *", parentCapId);
                var heatIns = getAppSpecific("Heater Installation *", parentCapId);
                var filPump = getAppSpecific("Changing the filter and/or pump *", parentCapId);
                var chemStor = getAppSpecific("Changing chemical storage *", parentCapId);
                var chPipe = getAppSpecific("Changing Piping *", parentCapId);
                var poolSh = getAppSpecific("Renovation of Pool shell *", parentCapId);
                var bathReno = getAppSpecific("Bathhouse Facility Renovation", parentCapId);
                var depMark = getAppSpecific("Depth Markers Upgrade and/or Pool Coping Replace", parentCapId);
                var poolDeck = getAppSpecific("Pool Deck Upgrade", parentCapId);
                var fence = getAppSpecific("Replacing Fence", parentCapId);
                var marble = getAppSpecific("Marble Dusting", parentCapId);
                var heatIns = getAppSpecific("Heater Installation *", parentCapId);

                editAppSpecific4ACA("Water Supply", watSupply, amendCapModel);
                editAppSpecific4ACA("Sewerage System", sewSystem, amendCapModel);
                editAppSpecific4ACA("Change in Use", chUse, amendCapModel);
                editAppSpecific4ACA("Scope of Project", proSco, amendCapModel);
                editAppSpecific4ACA("Saltwater-Chlorine Generator Installation *", salt, amendCapModel);
                editAppSpecific4ACA("Upgrading Chemical Feeders *", chemFeed, amendCapModel);
                editAppSpecific4ACA("Heater Installation *", heatIns, amendCapModel);
                editAppSpecific4ACA("Changing the filter and/or pump *", filPump, amendCapModel);
                editAppSpecific4ACA("Changing chemical storage *", chemStor, amendCapModel);
                editAppSpecific4ACA("Changing Piping *", chPipe, amendCapModel);
                editAppSpecific4ACA("Renovation of Pool shell *", poolSh, amendCapModel);
                editAppSpecific4ACA("Bathhouse Facility Renovation", bathReno, amendCapModel);
                editAppSpecific4ACA("Depth Markers Upgrade and/or Pool Coping Replace", depMark, amendCapModel);
                editAppSpecific4ACA("Pool Deck Upgrade", poolDeck, amendCapModel);
                editAppSpecific4ACA("Replacing Fence", fence, amendCapModel);
                editAppSpecific4ACA("Marble Dusting", marble, amendCapModel);
                editAppSpecific4ACA("Heater Installation *", heatIns, amendCapModel);
            ...
            else if(appTypeArray[0] == "DEQ" && appTypeArray[1] == "WWM" && appTypeArray[2] == "Garbage" && appTypeArray[3] == "Amendment")
            {
              logDebugLocal("Copy Contacts for Garbage Amend from:" + parentCapId + " to " + targetCapId);
                ...
                var amendCapModel = aa.cap.getCapViewBySingle4ACA(targetCapId);
                useAppSpecificGroupName = false;
                var offMat = getAppSpecific("Type of offensive material to be handled (Household Garbage, C&D, etc)", parentCapId);
                var totTruck = getAppSpecific("Total number of trucks", parentCapId);
                var servArea = getAppSpecific("Areas to be served (Hamlets, Villages, Towns)", parentCapId);
                var disIns = getAppSpecific("Disability insurance policy number", parentCapId);
                var disCom = getAppSpecific("Disability insurance company", parentCapId);
                var disDate = getAppSpecific("Disability insurance expiration date", parentCapId);
                ...
                logDebugLocal("Update ASI for offMat ACA:" + offMat);
                logDebugLocal("Update ASI for totTruck ACA:" + totTruck);
                logDebugLocal("Update ASI for servArea ACA:" + servArea);
                logDebugLocal("Update ASI for disIns ACA:" + disIns);
                logDebugLocal("Update ASI for disDate  ACA:" + disDate );
                logDebugLocal("Update ASI for workPol ACA:" + workPol);
                logDebugLocal("Update ASI for workCom ACA:" + workCom);
                logDebugLocal("Update ASI for workDate ACA:" + workDate);
                ...
                editAppSpecific4ACA("Areas to be served (Hamlets, Villages, Towns)", servArea, amendCapModel);
                editAppSpecific4ACA("Disability insurance policy number", disIns, amendCapModel);
                editAppSpecific4ACA("Disability insurance company", disCom, amendCapModel);
                editAppSpecific4ACA("Disability insurance expiration date", disDate, amendCapModel);
                ...
                logDebugLocal("Set CapModel:");
                aa.env.setValue("CapModel", amendCapModel);
                logDebugLocal("Set CAP_MODEL_INITED:");
                aa.env.setValue("CAP_MODEL_INITED", "TRUE");   
                logDebugLocal("Done!!!");
            }
        }
...
function copyAppSpecificTableForDEQ(srcCapId, targetCapId)
{
	...
	for (loopk in tableNameArray)
	{
		...
		if (IsStrInArry(tableName,targetTableNameArray)) { 
			...
			var sourceAppSpecificTable = getAppSpecificTableForDEQ(srcCapId,tableName);
...
function copyContactsSwim(pFromCapId, pToCapId) {
