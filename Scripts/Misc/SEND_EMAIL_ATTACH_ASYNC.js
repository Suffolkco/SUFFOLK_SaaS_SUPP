try {
	...
	var vChangeReportDescription = aa.env.getValue("vChangeReportDescription");
	...
	if (reportTemplate != '' && reportTemplate != null) {
		 ...
		 */
		//update the report description if one was provided. this will be used to update the saved report's description
		if (vReportName != false && vChangeReportDescription != null && vChangeReportDescription != "") {
			editDocumentDescription(vReportName, vChangeReportDescription);
		}
	}
	...
	for (w in emailArray) {
		...
		//Send email
		//Check ENVIRO global variable and only send to verified domains for Non-Production Environments
		var vOriginalDomain;
		if (typeof ENVIRON != "undefined" && typeof WhiteListEmailDomains != "undefined") {			
			if (ENVIRON != "PROD") {				
				//replace domain in email address
				conEmail = conEmail + "";
				vOriginalDomain = conEmail.substring(conEmail.indexOf("@") + 1, conEmail.length);		
				if (exists(vOriginalDomain, WhiteListEmailDomains) == false) {
					logDebug("Non-production environment. Changing email domain for " + conEmail);
					conEmail = conEmail.replace(vOriginalDomain, "DoNotSend.com");
				}
			}
		}
		//Check if email is in the EXCLUDED_EMAIL_ADDRESSES standard choice
		var vExcludedEmails = lookup("EXCLUDED_EMAIL_ADDRESSES","EXCLUDED_EMAIL_ADDRESSES");
		if (vExcludedEmails != null && vExcludedEmails != "") {
			vExcludedEmails = vExcludedEmails.split(",");
			if (exists(conEmail, vExcludedEmails) == true) {
				//replace domain in email address
				conEmail = conEmail + "";
				vOriginalDomain = conEmail.substring(conEmail.indexOf("@") + 1, conEmail.length);						
				logDebug("Email is in the EXCLUDED_EMAIL_ADDRESSES standard choice. Changing email domain for " + conEmail);
				conEmail = conEmail.replace(vOriginalDomain, "DoNotSend.com");				
			}
		}				
